function plot_2d_9panel_obs_mod_diff(p)
%str=vname(end-2:end);
...
elseif (p.optit==1)
  ...
  s3=sprintf('%s (mean:%5.2f %s)',s3,z3_mean,p.un); 
  s4=sprintf('%s (mean:%5.2f %s)',s4,z4_mean,p.un);
  s5=sprintf('%s (mean:%5.2f %s)',s5,z5_mean,p.un); 
  s6=sprintf('%s (mean:%5.2f %s)',s6,z6_mean,p.un); 
  s7=sprintf('%s (mean:%5.2f %s)',s7,z7_mean,p.un); 
  s8=sprintf('%s (mean:%5.2f %s)',s8,z8_mean,p.un); 
  s9=sprintf('%s (mean:%5.2f %s)',s9,z9_mean,p.un); 
%  s8=sprintf('%s (mean:%5.2f%%)',s8,z8_mean); 
elseif (p.optit==2)
  s1=sprintf('%s (mean:%5.2f %s)',s1,z1_mean,p.un);
  s2=sprintf('%s (mean:%5.2f %s)',s2,z2_mean,p.un);
...
pms=[ 0, 0, 1400, 800]*1; lw=2; row=3; col=3;
handle = figure('Position', pms,'visible','on');
subplot(row,col,1); 
pcolor(x,y,z1); hold on; shading flat; caxis([p.c1]); 
...
subplot(row,col,2); 
pcolor(x,y,z2); hold on; shading flat; caxis([p.c2]); 
contour(x,y,lm,1,p.c); axis(xy);
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
%s6=sprintf('%s (mean:%5.3f; %5.1f%%)',s6,z6_mean,z6_mean/z0_mean*100); 
title(s2,'FontSize',fsize); 
subplot(row,col,3); id=abs(z3)<p.th; z3(id)=NaN;
pcolor(x,y,z3); hold on; shading flat; caxis([p.c3]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
...
subplot(row,col,4); 
pcolor(x,y,z4); hold on; shading flat; caxis([p.c4]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
...
subplot(row,col,5); 
pcolor(x,y,z5); hold on; shading flat; caxis([p.c5]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
%s4=sprintf('%s (mean:%5.3f; %5.1f%%)',s4,z4_mean,z4_mean/z0_mean*100); 
title(s5,'FontSize',fsize); 
subplot(row,col,6); id=abs(z6)<p.th; z6(id)=NaN;
pcolor(x,y,z6); hold on; shading flat; caxis([p.c6]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
...
subplot(row,col,7); 
pcolor(x,y,z7); hold on; shading flat; caxis([p.c7]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c); title(s7,'FontSize',fsize); 
subplot(row,col,8);
pcolor(x,y,z8); hold on; shading flat; caxis([p.c8]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c); title(s8,'FontSize',fsize); 

cb1=colorbar('FontSize',fsize+2,'Location','southoutside');
set(cb1, 'Position', [.18 .04 .4 .02]); caxis([p.c7]); 
cb1.Label.String = p.un; pos=get(cb1,'Position')
cb1.Label.Position = [pos(1) pos(2)+2]; 

subplot(row,col,9); id=abs(z9)<p.th; z9(id)=NaN;
pcolor(x,y,z9); hold on; shading flat; caxis([p.c9]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c); title(s9,'FontSize',fsize); 

cb2=colorbar('FontSize',fsize+2,'Location','southoutside');
set(cb2, 'Position', [.7 .04 .2 .02]); caxis([p.c9]); 
cb2.Label.String = p.un; pos=get(cb2,'Position')
cb2.Label.Position = [pos(1)-0.4 pos(2)+2]; 
