function v=readts_grid(tpath, expn, fname, latlon)
...
v.time=f{'time'}(:);  v
v.time=v.time-v.time(1); v.nt=length(v.time);
v.ts=1; v.te=v.nt;    
v.pf=f{'pfull'}(:); v.nz=length(v.pf); %v.nz=min(find(v.pf<70));
v.zs=1; v.ze=v.nz;
close(f);
...
fname=strcat(tpath,expn,'/atmos.static.nc')
