...
tpath='/archive/Ming.Zhao/awg/warsaw/c96L33_am4p0/ts_all_aero/';
fn=strcat(tpath,'atmos_month_aer.2000-2014.ann.nc');
f=netcdf(fn,'nowrite'); lat=f{'lat'}(:); lon=f{'lon'}(:);
varn='aer_ex_c_vs'; z1=f{varn}(:,:); unit=''; 
...
[lonm,latm]=meshgrid(lon,lat); id_nh=(latm>0); id_sh=~id_nh;%figure; pcolor(id_nh);shading flat; colorbar;
id=id_nh; a(id)=a(id)/mean(a(id)); z1avg.nh=mean(z1(id).*a(id));
id=id_sh; a(id)=a(id)/mean(a(id)); z1avg.sh=mean(z1(id).*a(id));

tpath='/archive/Ming.Zhao/awg/AM3/AM3/ts_all_aero/';
fn=strcat(tpath,'atmos_month_aer.2000-2014.ann.nc');
f=netcdf(fn,'nowrite'); lat=f{'lat'}(:); lon=f{'lon'}(:);
varn='aer_ex_c_vs'; z2=f{varn}(:,:); unit=''; 
idl=(am4.v0.s.lm0==1); ido=~idl; a=am4.v0.s.aa; 
z2avg.globe=mean(mean(z2.*a));
id=ido; a(id)=a(id)/mean(a(id)); z2avg.ocean=mean(z2(id).*a(id));
id=idl; a(id)=a(id)/mean(a(id)); z2avg.land =mean(z2(id).*a(id));
[lonm,latm]=meshgrid(lon,lat); id_nh=(latm>0); id_sh=~id_nh;%figure; pcolor(id_nh);shading flat; colorbar;
id=id_nh; a(id)=a(id)/mean(a(id)); z2avg.nh=mean(z2(id).*a(id));
id=id_sh; a(id)=a(id)/mean(a(id)); z2avg.sh=mean(z2(id).*a(id));


tpath='/archive/Ming.Zhao/awg/AM2.1/AM2.1/ts_all_aero/';
fn=strcat(tpath,'atmos_month_aer.2000-2014.ann.nc');
f=netcdf(fn,'nowrite'); lat=f{'lat'}(:); lon=f{'lon'}(:);
varn='aer_ex_c_vs'; z3=f{varn}(:,:); unit=''; 
idl=(am4.v0.s.lm0==1); ido=~idl; a=am4.v0.s.aa; 
z3avg.globe=mean(mean(z3.*a));
id=ido; a(id)=a(id)/mean(a(id)); z3avg.ocean=mean(z3(id).*a(id));
id=idl; a(id)=a(id)/mean(a(id)); z3avg.land =mean(z3(id).*a(id));
[lonm,latm]=meshgrid(lon,lat); id_nh=(latm>0); id_sh=~id_nh;%figure; pcolor(id_nh);shading flat; colorbar;
id=id_nh; a(id)=a(id)/mean(a(id)); z3avg.nh=mean(z3(id).*a(id));
id=id_sh; a(id)=a(id)/mean(a(id)); z3avg.sh=mean(z3(id).*a(id));
...
s2=['MODIS retrieval (2003-2015); ' s2];
...
%cbunits(h,unit);
title('AM4.0 minus MODIS','FontSize',fsize);
