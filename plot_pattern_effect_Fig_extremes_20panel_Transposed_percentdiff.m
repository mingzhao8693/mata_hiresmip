function plot_pattern_effect_Fig_extremes_20panel_Transposed_percentdiff(p)
...
for r = 1:5 % Rows: Present, SPEAR, Observed, Diff
    for c = 1:4 % Cols: TAS, VPD, TWB, RH
	  ...
	  elseif r==2; a=p.v0; b=p.dv0; sx=let(idx)+p.s0; cax=[p.cmin0 p.cmax0];
          ...
          else; a=(p.v2-p.v1)./p.v1*100; b=p.dv2-p.dv1; sx=let(idx)+'(m) minus (i)'; cax=[p.cmin1 p.cmax1];
          end;
	  ...
	  elseif r==2; a=p.v3; b=p.dv3; sx=let(idx)+p.s3; cax=[p.cmin3 p.cmax3];
          ...
          else;  a=(p.v5-p.v4)./p.v4*100;  b=p.dv5-p.dv4; sx=let(idx)+'(n) minus (j)'; cax=[p.cmin4 p.cmax4];
          end;
	  ...
	  elseif r==2; a=p.v6; b=p.dv6; sx=let(idx)+p.s6; cax=[p.cmin6 p.cmax6];
          ...
          else; a=(p.v8-p.v7)./p.v7*100; b=p.dv8-p.dv7; sx=let(idx)+'(o) minus (k)'; cax=[p.cmin7 p.cmax7];
          end;
	  ...
	  elseif r==2; a=p.v9;  b=p.dv9; sx=let(idx)+p.s9; cax=[p.cmin9 p.cmax9];
          ...
          else; a=(p.v11-p.v10)./p.v10*100;  b=p.dv11-p.dv10; sx=let(idx)+'(p) minus (l)'; cax=[p.cmin10 p.cmax10];
          end;
