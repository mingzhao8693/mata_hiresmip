function plot_2d_6panel_frqday(p)
%str=vname(end-2:end);
...
pms=[ 0, 0, 900, 700]*1; lw=2; row=3; col=2;
...
% $$$ subplot(row,col,7); 
% $$$ pcolor(x,y,z7); hold on; shading flat; colorbar; caxis([p.c7]); 
% $$$ contour(x,y,lm,1,p.c); axis(xy); 
% $$$ ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
% $$$ myboxon(0,360,-90,90,p.c); title(s7,'FontSize',fsize); 
% $$$ subplot(row,col,8); z8(z8>95)=95;
% $$$ pcolor(x,y,z8); hold on; shading flat; colorbar; caxis([p.c8]); 
% $$$ contour(x,y,lm,1,p.c); axis(xy); 
% $$$ ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
