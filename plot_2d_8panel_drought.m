function plot_2d_8panel_drought(p)
...
mk='.'; ms=10; mc=0.1*[1 1 1]; ds=100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pms=[ 0, 0, 1000, 1000]*1; fsize=9; row=4; col=2; minus=char(8212);
handle = figure('Position', pms,'visible','on'); i=1; unit=p.unit;
x0=0.08; y0=0.8; wx=0.4; wy=0.18; dx=0.05+0.0; dy=0.05;
...
p7= [x0,          y0-3*(wy+dy), wx, wy];%position of first subplot
p8= [x0+wx+dx,    y0-3*(wy+dy), wx, wy];%position of first subplot
...
title(s,'FontSize',fsize,'Interpreter','latex');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot('Position',p7); a=p.v7; b=p.dv7; isig=p.isig7; sx=let(i)+p.s5; i=i+1; 
pcolor(p.lon,p.lat,a); hold on; shading flat; caxis([cmin1 cmax1]);
if p.do_add; contour(p.lon,p.lat,p.v1_add,p.v1_addbin,p.co_add,'showtext',p.show);end;
contour(p.lon,p.lat,p.lm,1,co); [xx,yy] = meshgrid(lon,lat);
stipple(xx,yy,isig,'density',ds,'color',mc,'marker',mk,'markersize',ms); 
s=sprintf('%s%s (%5.3f%s)',sx,p.varn,b,p.unit0); %s=sprintf('%s',sx); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot('Position',p8); a=p.v8; b=p.dv8; isig=p.isig8; sx=let(i)+p.s6; i=i+1; 
pcolor(p.lon,p.lat,a); hold on; shading flat;  caxis([cmin1 cmax1]);
if p.do_add; contour(p.lon,p.lat,p.v1_add,p.v1_addbin,p.co_add,'showtext',p.show);end;
contour(p.lon,p.lat,p.lm,1,co); [xx,yy] = meshgrid(lon,lat);
stipple(xx,yy,isig,'density',ds,'color',mc,'marker',mk,'markersize',ms); 
...
cb.Label.Position = [pos(1)-0.16 pos(2)+2]; 
cb.Label.Rotation = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
