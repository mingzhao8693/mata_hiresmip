...
clm=squeeze(mean(a,2)); clm_std=squeeze(std(a,1,2));
ann=squeeze(mean(a,1));
b=a(1:12,:,:,:); b=reshape(b,nyr*12,v.nlat,v.nlon); sea_std(1,:,:)=std(b,1);
b=a(3:5, :,:,:); b=reshape(b,nyr*3, v.nlat,v.nlon); sea_std(2,:,:)=std(b,1);
b=a(6:8, :,:,:); b=reshape(b,nyr*3, v.nlat,v.nlon); sea_std(3,:,:)=std(b,1);
b=a(9:11,:,:,:); b=reshape(b,nyr*3, v.nlat,v.nlon); sea_std(4,:,:)=std(b,1);
b=[a(12,:,:,:); a(1,:,:,:); a(2,:,:,:)];
b=reshape(b,nyr*3,v.nlat,v.nlon); sea_std(5,:,:)=std(b,1);
...
%  'In extracts: this is c48 model run, no intepolation needed'
  ...
  p.clm_std=clm_std;
  p.sea_std1=sea_std;
  'In extracts: always do intepolating...'
  ...
  p.clm_std=interp_grid(clm_std,lonx,latx,v.lon,v.lat,1);
  p.sea_std1=interp_grid(sea_std,lonx,latx,v.lon,v.lat,1);
else
...
p.sea_std=get4season(p.clm_std);
...
v.p.do_all
if (opt==1 & v.p.do_all) 
  tmp=reshape(tmp,12,v.nt/12,v.nlat,v.nlon); o=v.s;
  p.all=interp_grid(tmp,o.lon,o.lat,v.lon,v.lat,1);
  p.al0=getts(p.all,o); 
end
