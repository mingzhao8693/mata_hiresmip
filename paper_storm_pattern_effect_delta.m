...
p.vname='dsst_dfrqday'; p.cmin1=-1.5; p.cmax1=1.5; p.cmin2=-3; p.cmax2=3;p.co='k';
...
v=z.w0; a=v.sfc.tsurf.tavg0-a0; p.v3=a; p.dv3=mean(mean(a(id).*aa0));p.v3=(a-p.dv3)/p.dv3;
p.v1(~id)=NaN; p.v2(~id)=NaN; p.v3(~id)=NaN; p.v1_add(~id)=NaN;
v=v0.ar; p.lon=v.lon; p.lat=v.lat; p.lm=v.lm; aa0=v.aa; id=~isnan(aa);
a0=v0.ar.frqday;
v=w1; a=squeeze(v.ar.frqday-a0)*100/p.dT1; p.v4=a; p.dv4=mean(mean(a.*aa0));
v=w2; a=squeeze(v.ar.frqday-a0)*100/p.dT2; p.v5=a; p.dv5=mean(mean(a.*aa0));
v=w0; a=squeeze(v.ar.frqday-a0)*100/p.dT3; p.v6=a; p.dv6=mean(mean(a.*aa0));
a0=v0.tc.frqday;
v=w1; a=squeeze(v.tc.frqday-a0)*100/p.dT1; p.v7=a; p.dv7=mean(mean(a.*aa0));
v=w2; a=squeeze(v.tc.frqday-a0)*100/p.dT2; p.v8=a; p.dv8=mean(mean(a.*aa0));
v=w0; a=squeeze(v.tc.frqday-a0)*100/p.dT3; p.v9=a; p.dv9=mean(mean(a.*aa0));
a0=v0.mc.frqday;
v=w1; a=squeeze(v.mc.frqday-a0)*100/p.dT1; p.v10=a; p.dv10=mean(mean(a.*aa0));
v=w2; a=squeeze(v.mc.frqday-a0)*100/p.dT2; p.v11=a; p.dv11=mean(mean(a.*aa0));
v=w0; a=squeeze(v.mc.frqday-a0)*100/p.dT3; p.v12=a; p.dv12=mean(mean(a.*aa0));
plot_2d_8panel_wp_Fig1(p); %plot_2d_12panel_wp_Fig1(p);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p.let=["(a) ","(b) ","(c) ","(d) ","(e) ","(f) ","(g) ","(h) ","(i) ","(j) "...
     "(k) ","(l) ","(m) ","(n) ","(o) ","(p) ","(q) ","(r) ","(s) ","(t) "];
...
a0=z.v0.sfc.pcp.tavg0; p.v1_add=a0; p.v1_addbin=[1 5 9], p.v1_co='c';
a=(z.w1.sfc.pcp.tavg0-a0)/p.dT1; p.v1=a; p.dv1=mean(mean(a.*aa0));
a=(z.w2.sfc.pcp.tavg0-a0)/p.dT2; p.v2=a; p.dv2=mean(mean(a.*aa0));
a=(z.w0.sfc.pcp.tavg0-a0)/p.dT3; p.v3=a; p.dv3=mean(mean(a.*aa0));
v=v0.ar; p.lon=v.lon; p.lat=v.lat; p.lm=v.lm; aa0=v.aa; id=~isnan(aa);
a0=v0.ar.prday_idwei;
a=squeeze(w1.ar.prday_idwei-a0)/p.dT1; p.v4=a; p.dv4=mean(mean(a.*aa0));
a=squeeze(w2.ar.prday_idwei-a0)/p.dT2; p.v5=a; p.dv5=mean(mean(a.*aa0));
a=squeeze(w0.ar.prday_idwei-a0)/p.dT3; p.v6=a; p.dv6=mean(mean(a.*aa0));
a0=v0.tc.prday_idwei;
a=squeeze(w1.tc.prday_idwei-a0)/p.dT1; p.v7=a; p.dv7=mean(mean(a.*aa0));
a=squeeze(w2.tc.prday_idwei-a0)/p.dT2; p.v8=a; p.dv8=mean(mean(a.*aa0));
a=squeeze(w0.tc.prday_idwei-a0)/p.dT3; p.v9=a; p.dv9=mean(mean(a.*aa0));
a0=v0.mc.prday_idwei;
a=squeeze(w1.mc.prday_idwei-a0)/p.dT1; p.v10=a; p.dv10=mean(mean(a.*aa0));
a=squeeze(w2.mc.prday_idwei-a0)/p.dT2; p.v11=a; p.dv11=mean(mean(a.*aa0));
a=squeeze(w0.mc.prday_idwei-a0)/p.dT3; p.v12=a; p.dv12=mean(mean(a.*aa0));
plot_2d_8panel_wp_Fig2(p); %plot_2d_12panel_wp_Fig2(p);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p.let=["(a) ","(b) ","(c) ","(d) ","(e) ","(f) ","(g) ","(h) ","(i) ","(j) "...
     "(k) ","(l) ","(m) ","(n) ","(o) ","(p) ","(q) ","(r) ","(s) ","(t) "];
