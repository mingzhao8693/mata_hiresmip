...
from sfno_model import run_toolbox_sfno  # Clean import of the original SFNO pipeline

if __name__ == "__main__":
#ex: python sfno_training_amip.py z500 JJA  dataset  val_split_pct use_memory(0) sub_ens_size(5) num_draws (8) lat_min(15.0) lat_max(75.0)
#ex: python sfno_training_amip.py z500 JJA  0        0             0             5               8             15.0          75.0         
#ex: python sfno_training_cm4.py  z500 JJA  4        0.15          0             1               1             15.0          75.0         
    varn   = sys.argv[1]
    season = sys.argv[2]
    ...
    if dataset == 0:
        ...
        # 1 member of CM4_pi_Control simulation from 1 to 600 inclusive
        n_members = 1;   n_years = 600; yearlst=np.arange(1, 600);    fn='/work/miz/mat_ml/CM4_piControl_C.mat'
    ...
    # Combine back together for the standard 1D sample loading expected by SFNO wrapper
    ...
    #foundation_ckpt_path = f"/work/miz/mat_hiresmip/fno_gf/sfno_pretrained_{n_years}yr_{varn}_{season}_lat{int(lat_min)}to{int(lat_max)}.pt"
    foundation_ckpt_path = f"/work/miz/mat_hiresmip/fno_gf/sfno_pretrained_145yr_{varn}_{season}_lat{int(lat_min)}to{int(lat_max)}.pt"
    ...
    output_png = f"sfno_verification_comparison_9panel_train_val_{season}.png"
