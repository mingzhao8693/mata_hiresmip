...
from fno_model import FNOClimate2d
...
# --- FIX: Force Symmetrical Colorbar Limits Around Zero ---
abs_max = max(np.max(np.abs(z500_true_original)), np.max(np.abs(z500_ml_pred)))
vmin = -40 #-abs_max
vmax =  40 # abs_max

# Render Side-by-Side Spatial Comparisons
fig, axes = plt.subplots(2, 1, figsize=(30, 6), sharex=True, sharey=True)
...
axes[0].contour(lon,lat,lm,levels=[0.5],colors='k',linewidths=0.8)
...
axes[1].contour(lon,lat,lm,levels=[0.5],colors='k',linewidths=0.8)
