function v=rceana_ins_new(tpath,expn,res,ts,te,zs,ze)
[CPD,CPV,CL,RV,RD,LV0,G,ROWL,CPVMCL,EPS,EPSI,GINV,RDOCP,T0,HLF]=thermconst;
tpath='/archive/Ming.Zhao/ndrce/warsaw_201710/'; 
%expn='RCE_H2048x128_L79_R03_300'; res='org';  ts=300; te=301; zs=1;
expn='RCE_H0128x008_L79_R48_300'; res='org_z'; ts=200; te=201; zs=1;ze=64;
...
dp=rho.*dz*G; v.dp=getavar(dp,v,'3d');
...
%below for check%%%%%%%%%%%%%%%%%%%%%%%%%
%below as a check of pf ph which is not really needed for fixed zf0 cordinate
ck.ph_hy=ph_hy; ck.pf_hy=pf_hy;
ck.ph_hy(:,1,:,:)=v.bk(1)*ps+v.pk(1);
for k=2:v.nz_ph
  ck.ph_hy(:,k,:,:)=v.bk(k)*ps+v.pk(k);
  ck.pf_hy(:,k-1,:,:)=(ck.ph_hy(:,k,:,:) -    ck.ph_hy(:,k-1,:,:))./...
                  (log(ck.ph_hy(:,k,:,:))-log(ck.ph_hy(:,k-1,:,:)));
end
%below as a check of zf zh which is not really needed for fixed zf0 cordinate
varn='delz'; fname=strcat(tpath,expn,dir,varn,'_atmos_instant.nc')  
f=netcdf(fname,'nowrite'); a=f{varn}(ts:te,:,:,:); close(f); ck.dz=a;
ck.zh=a; ck.zf=a; ck.zh(:,v.nz+1,:,:)=0;
for k = v.nz : -1: 1 %a=dz(:,k,:,:);
  ck.zh(:,k,:,:)= ck.zh(:,k+1,:,:)+ck.dz(:,k,:,:);
  ck.zf(:,k,:,:)=(ck.zh(:,k+1,:,:)+ck.zh(:,k,:,:))*0.5;
end
ck.dp =diff(ck.ph_hy,1,2);
ck.rho=ck.dp./ck.dz/G;;
v.ck.dp0 =getavar(ck.dp, v,'3d');
v.ck.rho0=getavar(ck.rho,v,'3d');
v.ck.dz0 =getavar(ck.dz, v,'3d');
v.ck.zf0 =getavar(ck.zf, v,'3d');
%above for check%%%%%%%%%%%%%%%%%%%%%%%%%
...
k=36; a=squeeze(wa(:,k,:,:)); v.w500=getavar_smart(a,v,'2d',wbin);w500=a;
k=31; a=squeeze(wa(:,k,:,:)); v.w600=getavar_smart(a,v,'2d',wbin);w600=a;
k=26; a=squeeze(wa(:,k,:,:)); v.w700=getavar_smart(a,v,'2d',wbin);w700=a;
k=20; a=squeeze(wa(:,k,:,:)); v.w800=getavar_smart(a,v,'2d',wbin);w800=a;
