function v=clivar_ana_spearhi8(tpath,expn,latlon,region,yr1,yr2,syr,nyr,opt)
...
fn=strcat(tpath,expn,'/atmos.static.c192.nc'); g=get_grid(fn,latlon); g.opt=opt;
...
v.lat =v.glat(v.ys:v.ye); v.lon =v.glon(v.xs:v.xe); g.lat=v.lat; g.lon=v.lon; v.g=g;
...
f=netcdf(fn,'nowrite');
k=1; %1000hPa
a=f{varn}(v.ts:v.te,k,v.ys:v.ye,v.xs:v.xe); [a,v]=do_interp(a,g,v); size(a)
a(a==-1000)=NaN;
...
f=netcdf(fn,'nowrite');
a=f{varn}(v.ts:v.te,v.ys:v.ye,v.xs:v.xe); close(f); [a,v]=do_interp(a,g,v); size(a)
...
id=false(v.nlat,v.nlon); id_a=id; id_b=id; id_c=id; id_d=id; id_e=id;
for j=1:v.nlat
  for i=1:v.nlon
    a0=squeeze(a(j,i,:));
    b0=squeeze(b(j,i,:)); 
    ...
    if (sum(isnan(e0))==0); id_e(j,i)=true; end;
  end
end
N=20;
x=myeof(a,N,id_a); v.AO_z1000=x; 
x=myeof(b,N,id_b); v.AO_z850 =x;
x=myeof(c,N,id_c); v.AO_z700 =x;
x=myeof(d,N,id_d); v.AO_z500 =x;
x=myeof(e,N,id_e); v.AO_slp  =x;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%sample plots for EOFs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (do_sample_plot)
  x=v.AO_z700; lat=v.lat; lon=v.lon; i=1; z=-squeeze(x.eof_maps(:,:,i));
  ...
  figure; 
  plot(v.AO_z1000.index1,'r'); hold on; 
  plot(v.AO_z850.index1, 'g'); plot(v.AO_z700.index1, 'b'); 
  plot(v.AO_z500.index1, 'k'); plot(v.AO_slp.index1, 'c');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fn=strcat(tpath,expn,'/',expn,'_clivar_ana_spearhi8_',region,'.mat')
