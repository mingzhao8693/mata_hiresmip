function [v]=readarmap_pcp(tpath,expn,yr1,yr2,pct,opt)
...
for m=1:length(d_end)
  ...
  v.pr6hr_nonar_p99 (m,:,:) =s.nonar_p99;  %ar average
  v.pr6hr_nonar_p999(m,:,:) =s.nonar_p999; %ar average
  %PRC==========
  varn='prc'; ff='6hr'; exd='/atmos_data/6hr_avg/'; exf1='atmos_cmip.'; exf2='010100-'; exf3='123123.';
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff)*86400; 
  s=get_amon_var(a,id,pct,0,id99,id999,ip99,ip999);
  v.prc6hr_ar        (m,:,:) =s.ar;         %ar total
  v.prc6hr_armax     (m,:,:) =s.armax;      %ar max
  v.prc6hr_aravg     (m,:,:) =s.aravg;      %ar average
  v.prc6hr_aravg_p99 (m,:,:) =s.aravg_p99;  %ar average
  v.prc6hr_aravg_p999(m,:,:) =s.aravg_p999; %ar average
  v.prc6hr_nonar_p99 (m,:,:) =s.nonar_p99;  %ar average
  v.prc6hr_nonar_p999(m,:,:) =s.nonar_p999; %ar average
  %end of new addition
  %IVTY========
...
% $$$     a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); s=get_amon_var(a,id,pct,0,id99,id999,ip99,ip999);
	...
	id99(:,j,i)=idboth;
	ip99(:,j,i)=idallp & ~id_day(:,j,i);
	idallp=(a(:,j,i)>=P999(j,i));
	idboth=idallp & id_day(:,j,i);
	v.prday_fp999_ar(j,i)=sum(idboth)/sum(idallp);
	id999(:,j,i)=idboth;
	ip999(:,j,i)=idallp & ~id_day(:,j,i);
      end
    end
    v.idday_sum   =squeeze(sum(id_day,1));
    v.idday99_sum =squeeze(sum(id99, 1));
    v.idday999_sum=squeeze(sum(id999,1));
    v.ipday99_sum =squeeze(sum(ip99, 1));
    v.ipday999_sum=squeeze(sum(ip999,1));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    varn='pr'; ff='day'; exd='/atmos_data/daily/'; exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
    a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff)*86400; 
    s=get_amon_var(a,id_day,pct,0,id99,id999,ip99,ip999);
...
fnmat=strcat(epath,expn,fext,'_readarmap_pcp.mat')
