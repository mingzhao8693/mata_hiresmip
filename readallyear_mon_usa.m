function [var,time]=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,opt,lev)
...
for t=1:nyr
  ...
  elseif strcmp(varn,'shape_ar')
    ...
    c=ncread(fn,'shape_td',[1 1 tbeg],[Inf Inf tcon]); c=cast(c,'double');
    id=(b>0)|(c>0); a(id)=0; %exclude TC and TD regions
%   size(id); figure; pcolor(squeeze(b(:,:,1))); shading flat; colorbar; max(max(max(a)))
...
%    id=(a>0)|(b>0)|(c>0)|(d>0); a(id)=1; %include all AR, TC and TD from OLR regions
  elseif (length(varn)>6 & strcmp(varn(end-5:end),'_unmsk'))
    ...
    if (exist(fn,'file') == 2)
      ...
      a=v.lm_org; [nlat nlon]=size(a);
      clear a; a(1:nlon,1:nlat,1:1460)=0;
    end
  end
  if (exist(fn,'file') == 2)
  ...
  a=permute(a,[3 2 1]);
  if (t==1)
    var=a(1:tcon,v.ys:v.ye,v.xs:v.xe);
  else
    var=cat(1,var,a(1:tcon,v.ys:v.ye,v.xs:v.xe));
  end
end
