function plot_pattern_effect_Fig_extremes_20panel(p)
%let=["(a) ","(b) ","(c) ","(d) ","(e) ","(f) ","(g) ","(h) ","(i) ","(j) "...
%     "(k) ","(l) ","(m) ","(n) ","(o) ","(p) ","(q) ","(r) ","(s) ","(t) "];
%[lat,lon]=borders('United States'); us.lat=lat; us.lon=lon; us.co='k';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pms=[ 0, 0, 1400, 1000]*1.2; fsize=9; row=4; col=4; minus=char(8212);
handle = figure('Position', pms,'visible','on'); i=1; unit=p.unit1;
nn=256;  cmap=bluewhitered_miz(nn); colormap(cmap); %colormap(jet); 
if p.flipcmap; colormap(flipud(cmap)); end; %sgtitle(upper(p.sea))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%v0 v1 v2 (v2-v1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%v3 v4 v5 (v5-v4)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot(row,col,i); a=p.v3; b=p.dv3; a(~p.id)=NaN; sx=let(i)+p.s3; i=i+1; a3=a; b3=b;
pcolor(lon,lat,a); hold on; shading flat;  caxis([p.cmin3 p.cmax3]);
contour(lon,lat,lm,1,co); axis(p.xy); %plot(us.lon,us.lat,us.co);
for k=1:length(us.lon); plot(us.lon{k},us.lat{k},us.co,'LineWidth',lw); end; 
[xx,yy] = meshgrid(lon,lat);
...
s=sprintf('%s (%5.3f%s)',sx,b,p.unit4); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=a5-a4; b=b5-b4; a(~p.id)=NaN; sx=let(i)+'(g) minus (f)'; i=i+1; 
pcolor(p.lon,p.lat,a); hold on; shading flat;  caxis([p.cmin4 p.cmax4]);
contour(p.lon,p.lat,p.lm,1,co); axis(p.xy); %plot(us.lon,us.lat,us.co);
for k=1:length(us.lon); plot(us.lon{k},us.lat{k},us.co,'LineWidth',lw); end; 
[xx,yy] = meshgrid(lon,lat);
...
%v6 v7 v8 (v8-v7)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot(row,col,i); a=p.v6; a(~p.id)=NaN; b=p.dv6; sx=let(i)+p.s6; i=i+1; a6=a; b6=b;
...
s=sprintf('%s (%5.3f%s)',sx,b,p.unit7); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=a8-a7; b=b8-b7; a(~p.id)=NaN; sx=let(i)+'(k) minus (j)'; i=i+1; 
pcolor(p.lon,p.lat,a); hold on; shading flat;  caxis([p.cmin7 p.cmax7]);
contour(p.lon,p.lat,p.lm,1,co); axis(p.xy); %plot(us.lon,us.lat,us.co);
for k=1:length(us.lon); plot(us.lon{k},us.lat{k},us.co,'LineWidth',lw); end; 
[xx,yy] = meshgrid(lon,lat);
...
%v9 v10 v11 (v11-v10)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot(row,col,i); a=p.v9; b=p.dv9; a(~p.id)=NaN; sx=let(i)+p.s9; i=i+1;
...
s=sprintf('%s (%5.3f%s)',sx,b,p.unit10); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=a11-a10; b=b11-b10; a(~p.id)=NaN; sx=let(i)+'(o) minus (n)'; i=i+1; 
pcolor(p.lon,p.lat,a); hold on; shading flat;  caxis([p.cmin10 p.cmax10]);
contour(p.lon,p.lat,p.lm,1,co); axis(p.xy); %plot(us.lon,us.lat,us.co);
for k=1:length(us.lon); plot(us.lon{k},us.lat{k},us.co,'LineWidth',lw); end; 
[xx,yy] = meshgrid(lon,lat);
...
expn='pattern_effect_16panel'; visfig='off'; figpath='./fig_land/'; %figpath='./fig_spearhi8/'; 
