function v=tsana(o,tpath,expn,yr1,yr2,latlon,region,syr,nyr,myr,mod,opt)
[CPD,CPV,CL,RV,RD,LV0,G,ROWL,CPVMCL,EPS,EPSI,GINV,RDOCP,T0,HLF]=thermconst;
% $$$ tpath='/archive/Ming.Zhao/CM4/warsaw_201710_om4_v1.0.1/'; syr=111; nyr=35; myr=1;
% $$$ latlon=[0 360 -90 90]; region='global'; opt='2'; 
% $$$ expn='CM4_historical'; mod='c96';  yr1='1850';yr2='2014';syr=131;nyr=35;
...
varn='rkm_uws'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
if (exist(fname,'file') == 2)
  f=netcdf(fname,'nowrite'); 
  tmp=f{varn}(v.ts:v.te,v.ys:v.ye,v.xs:v.xe); close(f);
  v.atm.rkm_uws=extracts(tmp,v,o.dummy,myr,0);
end
varn='rkm_uwd'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
if (exist(fname,'file') == 2)
  f=netcdf(fname,'nowrite'); 
  tmp=f{varn}(v.ts:v.te,v.ys:v.ye,v.xs:v.xe); close(f);
  v.atm.rkm_uwd=extracts(tmp,v,o.dummy,myr,0);
end
