...
expn='c96L33_am4p2_o1_2010cntl';yr1='0002'; yr2='0021'; nyr=20;
v=mjomse_ana(tpath,expn,yr1,yr2,latlon,nyr,region)
...
fn=strcat(tpath,expn,'/',expn,'_',region,'_mjomse.mat'); load(fn);v0=v;
...
fn=strcat(tpath,expn,'/',expn,'_',region,'_mjomse.mat'); load(fn);v1=v;
fn=strcat(tpath,expn,'/',expn,'_',region,'_mjo.mat');    load(fn);v=v;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tpath ='/archive/Ming.Zhao/AM4LO/'; region='iowpac';
expn='CM_c96L33_am4p0_o1_2010cntl_mse';
fn=strcat(tpath,expn,'/',expn,'_',region,'_mjomse.mat'); load(fn);v1=v;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%z1=v.pcp.lagslop.org_avg'; z2=v.hfint0.lagslop.org_avg'; z2=z2/10^6; 
contourf(x,y,z1,[-2.4:0.1:2.4]); hold on; shading flat; caxis([-2 2]); colorbar;
[C,h]=contour(x,y,z2,[0 :0.2:1],'k-', 'LineWidth',lw); clabel(C,h)
[C,h]=contour(x,y,z2,[-1:0.2:0],'k--','LineWidth',lw); clabel(C,h)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pms=[ 0, 0, 1200, 900]*1; warning off all
handle = figure('Position', pms,'visible','on'); N=100; lw=1;
y0= v.io.hdt_tot_int.org (1:N);  plot(y0,'k-*','LineWidth',lw); hold on;s0='<dMSE/dt>-total';
y1= v.io.tdt_rad_int0.org(1:N);  plot(y1,'r-', 'LineWidth',lw); s1='<dMSE/dt>-rad';
y2a=v.io.tdt_dif_int0.org(1:N); %plot(y2a,'g--'); s2a='sensible';
y2b=v.io.qvdt_dif_int0.org(1:N);%plot(y2b,'c:');  s2b='latent heat';
y2 =y2a+y2b;                     plot(y2,'g-','LineWidth',lw); s2='<dMSE/dt>-shflx';
y3 =v.io.hdt_vadv_int0.org(1:N); plot(y3,'c-', 'LineWidth',lw);s3='<dMSE/dt>-vadv';
y4a=v.io.hdt_hadv_int0.org(1:N);
y4b=v.io.hdt_dgz_int0.org(1:N); %figure; plot(x,y4a,'r'); hold on; plot(x,y4b,'b');
y4 =y4a+y4b;                     plot(y4,'b-','LineWidth',lw);s4='<dMSE/dt>-hadv';
y5a=v.io.hdp_dyn_int0.org(1:N); 
y5b=v.io.hdt_ddp_int0.org(1:N); 
y5c=v.io.hdt_adj_int0.org(1:N); 
y5 =y5a+y5b+y5c;                 plot(y5,'m-','LineWidth',lw);s5='<dMSE/dt>-mass';
y6 =y3+y4+y5;                   %plot(y6,'c-','LineWidth',lw); s6='<dMSE/dt>-dyn';
y7 =y1+y2+y6   ;                 plot(y7,'-ks','MarkerSize',12);s7='sum=rad+shflx+vadv+hadv';
%y8=v.io.hdt_dgz_adj.org(1:N);   plot(y8,'-r*');hold on;plot(y5b+y5c+y4b,'bs-');
%y9=y1+y2+y3+y4a+y5a+y8;         plot(y9,'ko','MarkerSize',12);s4='sum=rad+sfc+dyn';
legend(s0,s1,s2,s3,s4,s5,s7,3); grid on;
...
y3 =v.io.hdt_vadv_int0.comp_iso_avg;plot(x,y3,'b-', 'LineWidth',lw);s3='<dMSE/dt>-vadv';
y4a=v.io.hdt_hadv_int0.comp_iso_avg;
y4b=v.io.hdt_dgz_int0.comp_iso_avg; %figure; plot(x,y4a,'r'); hold on; plot(x,y4b,'b');
y4 =y4a+y4b;                        plot(x,y4,'c-','LineWidth',lw);s4='<dMSE/dt>-hadv';
...
pms=[ 0, 0, 1200, 900]*0.5; warning off all
handle = figure('Position', pms,'visible','on');
x=v.io.pcp.lgd; y=v.pf; fsize=12; cx=0.01; lw=1;
z1=(v.io.hdt_hadv.comp_iso_avg+v.io.hdt_dgz_phy.comp_iso_avg); 
z2=(v.io.hdt_vadv.comp_iso_avg);%zx=v.io.dpint.comp_clm_avg;figure;plot(sum(zx.*z2,1));
...
[C,h]=contour(x,y,z2,[-cx :0.001:0], 'k--','LineWidth',lw); clabel(C,h)
[C,h]=contour(x,y,z2,[0 0],          'k-', 'LineWidth',lw*2); clabel(C,h)
...
z1=v.io.tdt_rad.comp_iso_avg;%zx=v.io.dpint.comp_clm_avg;figure;plot(sum(zx.*z1,1));
...
handle = figure('Position', pms,'visible','on');
x=v.io.pcp.lgd; y=v.pf; fsize=12; cx=0.006; lw=1;
z1a=v.io.tdt_dif.comp_iso_avg;
z1b=v.io.qvdt_dif.comp_iso_avg; z1=z1a+z1b; z1(z1<-cx)=-cx;
z2=(v.io.hf.comp_iso_avg); %z2=z2/10^6;
...
z1a=v.io.hdp_dyn.comp_iso_avg;
z1b=v.io.hdt_ddp_phy.comp_iso_avg;
z1c=v.io.hdt_adj_phy.comp_iso_avg; z1=z1a+z1b+z1c;
z2a=v.io.ddp_phy.comp_iso_avg; 
z2b=v.io.ddp_dyn.comp_iso_avg;     z2=z2a+z2b;
...
handle = figure('Position', pms,'visible','on');
x=v.io.pcp.lgd; y=v.pf; fsize=12; cx=0.006; lw=1;
z1a=v.io.tdt_conv.comp_iso_avg; 
z1b=v.io.qdt_conv.comp_iso_avg; z1=z1a+z1b; z1(z1<-cx)=-cx;
z2a=v.io.tdt_conv.comp_clm_avg;
z2b=v.io.qdt_conv.comp_clm_avg; z2=z2a+z2b;
%zx=v.io.dpint.comp_clm_avg;figure;plot(sum(zx.*z2,1)); hold on;
contourf(x,y,z1,[-cx:0.001:cx]);hold on; shading flat; caxis([-cx cx]); colorbar;
