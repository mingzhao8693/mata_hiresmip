function v=csampling_state_onet(v,wa,qc,buo,rop,tap,qt,rh,hli,mse,dse,mf)
for k=1:v.nz
  v.tngrid=v.ngrid; k
  for t=1:v.nt
    ...
    v.up.buo(t,k,:) = x.buo;   v.up.rop(t,k,:) = x.rop;
    v.up.tap(t,k,:) = x.tap;   v.up.qt (t,k,:) = x.qt; 
    v.up.rh (t,k,:) = x.rh;    v.up.hli(t,k,:) = x.hli;
    v.up.mse(t,k,:) = x.mse;   v.up.dse(t,k,:) = x.dse;
    v.up.mf (t,k,:) = x.mf;    v.up.mff(t,k,:) = x.mff;
