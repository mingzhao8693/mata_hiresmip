function s=get_amon_day(a,id,pct,opt,id99,id999,ip99,ip999)
...
if exist('opt')
  if (opt==0)
    ...
    s.aravg_p99 =squeeze(nanmean(d,1));
    idx=id999; b=a.*idx; d=b; d(~idx)=NaN;
    s.aravg_p999=squeeze(nanmean(d,1));
    idx=ip99;  b=a.*idx; d=b; d(~idx)=NaN;
    s.nonar_p99 =squeeze(nanmean(d,1));
    idx=ip999; b=a.*idx; d=b; d(~idx)=NaN;
    s.nonar_p999=squeeze(nanmean(d,1));
  elseif (opt==1)
    b=a.*id; d=b; d(~id)=NaN;
    s.ar   =squeeze(mean   (b,1));
    s.aravg=squeeze(nanmean(d,1)); 
