function plot_pattern_effect_Figs_suppl(p)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pms=[ 0, 0, 1800, 800]*1; fsize=8; row=4; col=5; minus=char(8212);
...
cb0.Label.String = p.unit; pos=get(cb0,'Position');
cb0.Label.Position = [pos(1)-0.3 pos(2)+0.11]; 
cb0.Label.Rotation = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
s=sprintf('%s (%5.3f%s)',sx,b,p.unit0); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=p.v17; b=p.dv17; isig=p.isig17; sx=let(i)+p.s17; i=i+1; a17=a; b17=b;
pcolor(lon,lat,a); hold on; shading flat; caxis([cmin4 cmax4]);
contour(lon,lat,lm,1,co); 
stipple(xx,yy,isig,'density',ds,'color',mc,'marker',mk,'markersize',ms); 
s=sprintf('%s (%5.3f%s)',sx,b,p.unit0); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=p.v18; b=p.dv18; isig=p.isig18; sx=let(i)+p.s18; i=i+1; a18=a; b18=b;
pcolor(lon,lat,a); hold on; shading flat;  caxis([cmin4 cmax4]);
contour(lon,lat,lm,1,co); 
stipple(xx,yy,isig,'density',ds,'color',mc,'marker',mk,'markersize',ms); 
s=sprintf('%s (%5.3f%s)',sx,b,p.unit0); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=p.v19; b=p.dv19; isig=p.isig19; sx=let(i)+p.s19; i=i+1; a19=a; b19=b;
pcolor(lon,lat,a); hold on; shading flat; caxis([cmin4 cmax4]);
contour(lon,lat,lm,1,co); 
stipple(xx,yy,isig,'density',ds,'color',mc,'marker',mk,'markersize',ms); 
s=sprintf('%s (%5.3f%s)',sx,b,p.unit0); 
title(s,'FontSize',fsize,'Interpreter','latex');
subplot(row,col,i); a=p.v20; b=p.dv20; isig=p.isig20; sx=let(i)+p.s20; i=i+1; a20=a; b20=b;
pcolor(lon,lat,a); hold on; shading flat;  caxis([cmin4 cmax4]);
contour(lon,lat,lm,1,co); 
stipple(xx,yy,isig,'density',ds,'color',mc,'marker',mk,'markersize',ms); 
s=sprintf('%s (%5.3f%s)',sx,b,p.unit0); 
