function plot_2d_9panel_storm_cre(p)
%str=vname(end-2:end);
...
if (opt==100)
  ...
  s1=sprintf('%s (mean:%5.1fW/m2)',s1,z1_mean);
  s2=sprintf('%s (mean:%5.1fW/m2)',s2,z2_mean);
  s3=sprintf('%s (mean:%5.1fW/m2)',s3,z3_mean); 
  s4=sprintf('%s (mean:%5.1fW/m2)',s4,z4_mean);
  s5=sprintf('%s (mean:%5.1fW/m2)',s5,z5_mean); 
  s6=sprintf('%s (mean:%5.1fW/m2)',s6,z6_mean);
  s7=sprintf('%s (mean:%5.1fW/m2)',s7,z7_mean);
  s8=sprintf('%s (mean:%5.1fW/m2)',s8,z8_mean); 
  s9=sprintf('%s (mean:%5.1fW/m2)',s9,z9_mean);
end
%xy=[0 360 -90 90]; 
xl='longitude'; yl='latitude'; xy=p.xy; a_th1=0.1; a_th2=1;
fq_th=[0.01 0.10]; color='blue'; fs=5;
pms=[ 0, 0, 1700, 1000]*0.85; fsize=10; lw=2; row=3; col=3;
handle = figure('Position', pms,'visible','on');
subplot(row,col,1); a=z1; %id=abs(a)<a_th1; a(id)=NaN;
pcolor(x,y,a); hold on; shading flat; colorbar; caxis([p.c1]); 
[C h]=contour(x,y,p.frqday_ar,fq_th,'b');
clabel(C,h,'FontSize',fs,'Color',color);
...
[C h]=contour(x,y,p.frqday_tc,fq_th,'b'); 
clabel(C,h,'FontSize',fs,'Color',color);
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
...
pcolor(x,y,a); hold on; shading flat; colorbar; caxis([p.c4]); 
[C,h]=contour(x,y,p.frqday_mc,fq_th,'b');
clabel(C,h,'FontSize',fs,'Color',color);
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
