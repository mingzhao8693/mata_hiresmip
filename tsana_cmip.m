function v=tsana_cmip(o,tpath,expn,yr1,yr2,latlon,region,syr,nyr,myr,mod,opt)
...
pp='/ts_all/'; fext =strcat(anam,'.',yr1,'01-',yr2,'12.');
varn='ts'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
...
for t=1:v.nt; aa1(t,:,:)=v.aa(:,:); end; v.aa0=v.aa; v.aa=aa1;
...
%Surface
varn='tas'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
f=netcdf(fname,'nowrite'); 
tmp=f{varn}(v.ts:v.te,v.ys:v.ye,v.xs:v.xe);close(f);%tmp=tmp-273.15;
...
varn='wap'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
f=netcdf(fname,'nowrite'); 
...
%ATM MC_FULL
varn='mc'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
...
%ATM T
varn='ta'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
f=netcdf(fname,'nowrite'); 
...
%ATM q
varn='hus'; fname=strcat(tpath,expn,pp,fext,varn,'.nc')
f=netcdf(fname,'nowrite'); 
