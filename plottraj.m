function v1=plottraj(v1,year,latbnd,iwho)

fac=20; fsize=16;lw=2; msize=3;
no=0; num3=0; num4=0; %iwho=0; %latbnd=90;
trtmp=v1.tr; wind0=14;

pms=[ 0, 0, 1280, 800]*1.5; warning off all; 
handle = figure('Position', pms,'visible','on'); 
contourf(v1.lon, v1.lat, v1.lm,'g'); hold on;
for k=1:length(trtmp)
  ts =trtmp(k); ts1=ts;
  if (ts.windmax>29.5); ts.hur=1; end;
  for i=1:ts.num-1
...
%  if (ts.hur>=iwho & ts.yearmax==year & (ts.boxnumb==2|ts.boxnumb==3)&idd)
  ...
  elseif (ts.hur>=iwho & isempty(year))
    plot(ts.lon(1),ts.lat(1),'r*','MarkerSize',msize);hold on;
    no=no+1;
  end
end
v1.nyr=v1.tr(end).year(1)-v1.tr(1).year(1);
title(strcat('global annual total number = ',num2str(no/v1.nyr)));
