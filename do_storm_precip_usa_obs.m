function x=do_storm_precip_usa_obs(opt,yr1,yr2)
...
opt='AR'; yr1=1985; yr2=1986; nyr=yr2-yr1+1;  %year=198501-200812
tpath='/work/miz/mat_hiresmip/obs/'; 
expn='ERAI_6h_DATA'; s0='_readartcmcs_day_usa.mat'; 
nyr=yr2-yr1+1; ext=strcat(opt,'_diag0'); t=1
for t=1:nyr
  ...
  prday_fp99 (t,:,:,:) = v.prday_fp99_from_id;
  s.lat=v.lat; s.lon=v.lon; s.lm0=v.lm; s.aa=v.aa;
  for m=1:12
    daily(m).prday(t,:,:,:)=v.daily(m).prday;
    daily(m).id   (t,:,:,:)=v.daily(m).id;
    var=squeeze(daily(m).prday(t,:,:,:)); 
    sname='NWR';        daily(m).nwr(t,:)=get_reg_mean(sname,s,var);
    sname='WWR';        daily(m).wwr(t,:)=get_reg_mean(sname,s,var);
    sname='SWR';        daily(m).swr(t,:)=get_reg_mean(sname,s,var);
    sname='SSR';        daily(m).ssr(t,:)=get_reg_mean(sname,s,var);
    sname='NRP';        daily(m).nrp(t,:)=get_reg_mean(sname,s,var);
    sname='SER';        daily(m).ser(t,:)=get_reg_mean(sname,s,var);
    sname='OVR';        daily(m).ovr(t,:)=get_reg_mean(sname,s,var);
    sname='UMW';        daily(m).umw(t,:)=get_reg_mean(sname,s,var);
    sname='NER';        daily(m).ner(t,:)=get_reg_mean(sname,s,var);
    sname='Alaska';           daily(m).al (t,:)=get_reg_mean(sname,s,var);
    sname='Yukon Territory';  daily(m).yu (t,:)=get_reg_mean(sname,s,var);
    sname='British Columbia'; daily(m).bc (t,:)=get_reg_mean(sname,s,var);
    sname='Washington';       daily(m).wa (t,:)=get_reg_mean(sname,s,var);
    sname='Oregon';           daily(m).or (t,:)=get_reg_mean(sname,s,var);
    sname='California';       daily(m).ca (t,:)=get_reg_mean(sname,s,var);
  end
end
N=10;
for m=1:12
  nday=length(daily(m).prday(1,:,1,1));
  a=daily(m).nwr(:,:); daily(m).idx.nwr=find_index(a,N);
  a=daily(m).wwr(:,:); daily(m).idx.wwr=find_index(a,N);
  a=daily(m).swr(:,:); daily(m).idx.swr=find_index(a,N);
  a=daily(m).ssr(:,:); daily(m).idx.ssr=find_index(a,N);
  a=daily(m).nrp(:,:); daily(m).idx.nrp=find_index(a,N);
  a=daily(m).ser(:,:); daily(m).idx.ser=find_index(a,N);
  a=daily(m).ovr(:,:); daily(m).idx.ovr=find_index(a,N);
  a=daily(m).umw(:,:); daily(m).idx.umw=find_index(a,N);
  a=daily(m).ner(:,:); daily(m).idx.ner=find_index(a,N);
  a=daily(m).al (:,:); daily(m).idx.al =find_index(a,N);
  a=daily(m).yu (:,:); daily(m).idx.yu =find_index(a,N);
  a=daily(m).bc (:,:); daily(m).idx.bc =find_index(a,N);
  a=daily(m).wa (:,:); daily(m).idx.wa =find_index(a,N);
  a=daily(m).or (:,:); daily(m).idx.or =find_index(a,N);
  a=daily(m).ca (:,:); daily(m).idx.ca =find_index(a,N);
end

pms=[ 0, 0, 1800, 1200]*1.5; row=3; col=3; fs=12;
handle=figure('Position',pms,'visible','on');
m=1; a=daily(m); yr1=1984;
for i=1:9
  %r=a.idx.ca; sname='California'; [la,lo]=borders(sname); lo=360+lo;
  r=a.idx.bc; sname='British Columbia';     [la,lo]=borders(sname); lo=360+lo;
  t(i)=r.t(i); d (i)=r.d(i); mp(i)=r.maxv(i);
  pr(i,:,:)=squeeze(a.prday(t(i),d(i),:,:));
  id(i,:,:)=squeeze(a.id   (t(i),d(i),:,:));
  subplot(row,col,i); z=squeeze(pr(i,:,:)); y=squeeze(id(i,:,:));
  pcolor(s.lon,s.lat,z); shading flat; colorbar; colormap(jet); hold on;
  contour(s.lon,s.lat,y,[0 0.5],'r');
  contour(s.lon,s.lat,s.lm0,[0 0.5],'w'); plot(lo,la,'w');
  st=sprintf('%4d%02d%02d; (pr=%4.2f)',t(i)+yr1,m,d(i),mp(i)); caxis([0 80]); 
  title(st,'FontSize',fs); axis([180 240 50 70]);
end
...
v=x; 
epath='/work/miz/mat_hiresmip/pcp/obs/'; 
ext=strcat('obs_',opt,'_',num2str(yr1),'_',num2str(yr2));
