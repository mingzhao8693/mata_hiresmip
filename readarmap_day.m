function [v]=readarmap_day(tpath,expn,yr1,yr2,pct,opt)
...
for m=1:length(d_end)
  ...
  v.ar6hr_fq (m,:,:)=squeeze(mean(id,1))/4; 
  v.ar6hr_fqs(m,:,:)=squeeze(sum (id,1));
  %start of daily fields
  for k=1:length(id(:,1,1))/4
    n=(k-1)*4+1; 
    id_day(k,:,:)=id(n,:,:)+id(n+1,:,:)+id(n+2,:,:)+id(n+3,:,:);
  end
  id=id_day; id(id<=2)=0; id(id>2)=1; 
...
fnmat=strcat(epath,expn,fext,'_readarmap_day.mat')
