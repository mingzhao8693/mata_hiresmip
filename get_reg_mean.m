function x=get_reg_mean(sname,s,var)
%sname='California'; var=squeeze(daily(m).prday(t,:,:,:));
a=s.lm0; aa=s.aa; lat=s.lat; lon=s.lon;
if strcmp(sname,'NWR')
...
id=id_rg; N=length(var(:,1,1));
for i=1:N
  a=squeeze(var(i,:,:)); 
  id=id&~isnan(a);
  x(i)=sum(a(id).*aa(id))./sum(aa(id));
end
