function plot_2d_12panel_gf(p)
%str=vname(end-2:end);
x=p.x; y=p.y; lm=p.lm; aa=p.aa; id=p.id;
z1=p.z1;z2=p.z2;z3=p.z3;z4=p.z4;z5=p.z5;z6=p.z6;z7=p.z7;z8=p.z8;z9=p.z9;z10=p.z10;z11=p.z11;z12=p.z12;
s1=p.s1;s2=p.s2;s3=p.s3;s4=p.s4;s5=p.s5;s6=p.s6;s7=p.s7;s8=p.s8;s9=p.s9;s10=p.s10;s11=p.s11;s12=p.s12;
...
z10_mean=miznanmean(z10,id,aa,opt);
z11_mean=miznanmean(z11,id,aa,opt);
z12_mean=miznanmean(z12,id,aa,opt);
if (~isfield(p,'optit'))
...
elseif (p.optit==2)
  ...
  a=corrcoef(z10,z1);s10=sprintf('%s (avg=%5.2f; r=%5.2f)',s10,z10_mean,a(1,2)); 
  a=corrcoef(z11,z2);s11=sprintf('%s (avg=%5.2f; r=%5.2f)',s11,z11_mean,a(1,2)); 
  a=corrcoef(z12,z3);s12=sprintf('%s (avg=%5.2f; r=%5.2f)',s12,z12_mean,a(1,2)); 
else
  disp('no title');
end
%xy=[0 360 -90 90]; 
...
pms=[ 0, 0, 1700, 1200]*1.5; fsize=10; lw=2; row=4; col=3;
...
subplot(row,col,10); a=z10; %id=abs(a)<a_th1; a(id)=NaN;
pcolor(x,y,a); hold on; shading flat; colorbar; caxis([p.c4]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
%s3=sprintf('%s (mean:%5.3f; %5.1f%%)',s3,z3_mean,z3_mean/z0_mean*100); 
title(s10,'FontSize',fsize); 

subplot(row,col,11); a=z11; %id=abs(a)<a_th1; a(id)=NaN;
pcolor(x,y,a); hold on; shading flat; colorbar; caxis([p.c3]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
%s2=sprintf('%s (mean:%5.3f; %5.1f%%)',s2,z2_mean,z2_mean/z0_mean*100); 
title(s11,'FontSize',fsize); 

subplot(row,col,12); a=z12; %id=abs(a)<a_th1; a(id)=NaN;
pcolor(x,y,a); hold on; shading flat; colorbar; caxis([p.c4]); 
contour(x,y,lm,1,p.c); axis(xy); 
ylabel(yl,'FontSize',fsize); set(gca,'FontSize',fsize);
myboxon(0,360,-90,90,p.c);
%s3=sprintf('%s (mean:%5.3f; %5.1f%%)',s3,z3_mean,z3_mean/z0_mean*100); 
title(s12,'FontSize',fsize); 
