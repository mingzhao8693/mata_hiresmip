function [v]=readartcmcs_day_cre_new_mod(tpath,expn,yr1,yr2,pct,opt,diag)
...
%expn ='ERAI_6h_DATA'; yr1=2001; yr2=2008; opt='MC';
%pct  =[99 99.9]; diag=2;
%tpath='/archive/Ming.Zhao/awg/2022.03/';
%expn ='c192L33_am4p0_amip_HIRESMIP_HX'; yr1=2001; yr2=2001; opt='AR';
%pct  =[99 99.9]; diag=2;
...
for m=1:length(d_end)
  ...
  %precip%%%%%%%%%%%%%%%%%%%%%%%
  varn='pr'; ff='day'; exd='/atmos_data/daily/'; 
  exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); prt=a; a=a*86400.; %unit:mm/day
  ...
  elseif strcmp(opt,'P01')
    id=(a>=0.1);   %regions of precip >= 0.1mm/day
  elseif strcmp(opt,'P02')
    id=(a>=0.2);   %regions of precip >= 0.2mm/day
  elseif strcmp(opt,'PC')
    id=(a>=0.2) & (a<1); %regions of 0.2 <precip < 1mm/day
  elseif strcmp(opt,'PD')
    id=(a<0.2);    %regions of precip < 0.2mm/day
   elseif strcmp(opt,'P00')
    id=(a<0.1);    %regions of precip < 0.1mm/day
  elseif strcmp(opt,'PA')
    id=(a>=0.1) & (a<1); %regions of 0.1 < precip < 1mm/day
  ...
  tmp=reshape(id,v.nyr,365,nlat,nlon);
  v.frqday_all=squeeze(mean(tmp,2));
...
%  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff)*86400; 
  ...
  tmp=(prt~=0); a(tmp)=a(tmp)./prt(tmp); a(~tmp)=0;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.fprc_av        (m,:,:)  =s.av;         %all average
  v.fprc_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.fprc_idwei     (m,:,:)  =s.idwei;      %id average
  v.fprc_idwei_ann (m,:,:,:)=s.idwei_ann;  %id average
  v.fprc_idmax     (m,:,:)  =s.idmax;      %id max
  v.fprc_idavg     (m,:,:)  =s.idavg;      %id average
  v.fprc_idstd     (m,:,:)  =s.idstd;      %id average
  v.fprc_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.fprc_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.fprc_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.fprc_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.fprc_av_ts_day       (m,:)   =s.av_ts_day;
  v.fprc_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.fprc_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.fprc_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if (diag==0); break; end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  v.rlut_av_ann    (m,:,:,:)=s.av_ann;
  v.rlut_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.rlut_idwei     (m,:,:)  =s.idwei;      %id average
  v.rlut_idwei_ann (m,:,:,:)=s.idwei_ann;
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='rlutcs'; ff='day'; exd='/atmos_data/daily/'; exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); rlutcs=a; lwcf=a-rlut; clear rlut;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.rlutcs_av        (m,:,:)  =s.av;         %all average
  v.rlutcs_av_ann    (m,:,:,:)=s.av_ann;
  v.rlutcs_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.rlutcs_idwei     (m,:,:)  =s.idwei;      %id average
  v.rlutcs_idwei_ann (m,:,:,:)=s.idwei_ann;
  v.rlutcs_idmax     (m,:,:)  =s.idmax;      %id max
  v.rlutcs_idavg     (m,:,:)  =s.idavg;      %id average
  v.rlutcs_idstd     (m,:,:)  =s.idstd;      %id average
  v.rlutcs_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.rlutcs_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.rlutcs_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.rlutcs_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.rlutcs_av_ts_day       (m,:)   =s.av_ts_day;
  v.rlutcs_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.rlutcs_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.rlutcs_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  a=lwcf; s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.lwcf_av        (m,:,:)  =s.av;         %all average
  v.lwcf_av_ann    (m,:,:,:)=s.av_ann;
  v.lwcf_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.lwcf_idwei     (m,:,:)  =s.idwei;      %id average
  ...
  v.rsut_av_ann    (m,:,:,:)=s.av_ann;
  v.rsut_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.rsut_idwei     (m,:,:)  =s.idwei;      %id average
  v.rsut_idwei_ann (m,:,:,:)=s.idwei_ann;
  ...
  v.rsut_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.rsut_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.rsut_av_ts_day       (m,:)   =s.av_ts_day;
  v.rsut_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.rsut_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.rsut_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='rsdt'; ff='day'; exd='/atmos_data/daily/'; exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); rsdt=a;
  a=a-rsut; swabs=a;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.swabs_av        (m,:,:)  =s.av;         %all average
  v.swabs_av_ann    (m,:,:,:)=s.av_ann;
  v.swabs_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.swabs_idwei     (m,:,:)  =s.idwei;      %id average
  v.swabs_idwei_ann (m,:,:,:)=s.idwei_ann;
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='rsutcs'; ff='day'; exd='/atmos_data/daily/'; exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); rsutcs=a;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.rsutcs_av        (m,:,:)  =s.av;         %all average
  v.rsutcs_av_ann    (m,:,:,:)=s.av_ann;
  v.rsutcs_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.rsutcs_idwei     (m,:,:)  =s.idwei;      %id average
  v.rsutcs_idwei_ann (m,:,:,:)=s.idwei_ann;
  v.rsutcs_idmax     (m,:,:)  =s.idmax;      %id max
  v.rsutcs_idavg     (m,:,:)  =s.idavg;      %id average
  v.rsutcs_idstd     (m,:,:)  =s.idstd;      %id average
  v.rsutcs_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.rsutcs_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.rsutcs_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.rsutcs_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.rsutcs_av_ts_day       (m,:)   =s.av_ts_day;
  v.rsutcs_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.rsutcs_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.rsutcs_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  a=rsutcs-rsut; swcf=a;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.swcf_av        (m,:,:)  =s.av;         %all average
  v.swcf_av_ann    (m,:,:,:)=s.av_ann;
  v.swcf_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.swcf_idwei     (m,:,:)  =s.idwei;      %id average
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  a=swabs-olr; netrad=a; 
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.netrad_av        (m,:,:)  =s.av;         %all average
  v.netrad_av_ann    (m,:,:,:)=s.av_ann;
  v.netrad_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.netrad_idwei     (m,:,:)  =s.idwei;      %id average
  v.netrad_idwei_ann (m,:,:,:)=s.idwei_ann;
  v.netrad_idmax     (m,:,:)  =s.idmax;      %id max
  v.netrad_idavg     (m,:,:)  =s.idavg;      %id average
  v.netrad_idstd     (m,:,:)  =s.idstd;      %id average
  v.netrad_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.netrad_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.netrad_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.netrad_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.netrad_av_ts_day       (m,:)   =s.av_ts_day;
  v.netrad_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.netrad_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.netrad_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  a=rsdt-rsutcs-rlutcs; netradclr=a;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.netradclr_av        (m,:,:)  =s.av;         %all average
  v.netradclr_av_ann    (m,:,:,:)=s.av_ann;
  v.netradclr_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.netradclr_idwei     (m,:,:)  =s.idwei;      %id average
  v.netradclr_idwei_ann (m,:,:,:)=s.idwei_ann;
  v.netradclr_idmax     (m,:,:)  =s.idmax;      %id max
  v.netradclr_idavg     (m,:,:)  =s.idavg;      %id average
  v.netradclr_idstd     (m,:,:)  =s.idstd;      %id average
  v.netradclr_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.netradclr_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.netradclr_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.netradclr_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.netradclr_av_ts_day       (m,:)   =s.av_ts_day;
  v.netradclr_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.netradclr_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.netradclr_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  a=netrad-netradclr; %clear netrad netradclr;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.netcf_av        (m,:,:)  =s.av;         %all average
  v.netcf_av_ann    (m,:,:,:)=s.av_ann;
  v.netcf_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;    
  v.netcf_idwei     (m,:,:)  =s.idwei;      %id average
  v.netcf_idwei_ann (m,:,:,:)=s.idwei_ann;
  v.netcf_idmax     (m,:,:)  =s.idmax;      %id max
  v.netcf_idavg     (m,:,:)  =s.idavg;      %id average
  v.netcf_idstd     (m,:,:)  =s.idstd;      %id average
  v.netcf_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.netcf_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.netcf_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.netcf_idx_std   (m,:,:,:)=s.idx_std;    %id average
  v.netcf_av_ts_day       (m,:)   =s.av_ts_day;
  v.netcf_idwei_ts_day    (m,:)   =s.idwei_ts_day;
  v.netcf_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
  v.netcf_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if (diag==1); break; end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  v.tas_idwei     (m,:,:)  =s.idwei;      %id average
  ...
  if (diag==2); break; end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='ivty'; ff='day'; exd='/atmos_data/daily_avg/';  exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='ivtx'; ff='day'; exd='/atmos_data/daily_avg/';  exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
