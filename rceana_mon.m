function v=rceana_mon(tpath,expn,res,ts,te,zs,qc_th,do_uwc)
[CPD,CPV,CL,RV,RD,LV0,G,ROWL,CPVMCL,EPS,EPSI,GINV,RDOCP,T0,HLF]=thermconst;
%tpath='/archive/Ming.Zhao/ndrce/warsaw_201710/'; qc_th=0.00001;
%expn='RCE_H0128x008_L79_R48_300'; ts=8; te=12; zs=1; res='org';
%expn='RCE_H2048x128_L79_R03_300'; ts=5; te=12; zs=1; res='org';
...
zh=a; zf=a; zh(:,v.nz+1,:,:)=0;
for k = v.nz : -1: 1 %a=dz(:,k,:,:);
  zh(:,k,:,:)= zh(:,k+1,:,:)+dz(:,k,:,:);
  zf(:,k,:,:)=(zh(:,k+1,:,:)+zh(:,k,:,:))*0.5;
end
v.atm.zf = getavar(zf, v,'3d'); v.zf=v.atm.zf.avg/1000; v.zf0=v.zf;
v.atm.zh = getavar(zh, v,'3d'); v.zh=v.atm.zh.avg/1000; clear zh

varn='w';       a=f{varn}(ts:te,zs:ze,:,:); wa=a;
...
%below set conditional sampling criteria
id.up =(wa>0); id.cup=(wa>0 & qc>qc_th); id.cld=(qc> qc_th);
id.dn =(wa<0); id.cdn=(wa<0 & qc>qc_th); id.clr=(qc<=qc_th);
%partition large-scale circulation based on 500hPa w
k=44; %500hPa level
wa500=squeeze(wa(:,k,:,:)); qc500=squeeze(qc(:,k,:,:));
idx.up =(wa500>0); idx.dn =(wa500<0);

%below conditional sampling
v.wa=csampone(wa,v,'3d',id,1); 
v.om=csampone(om,v,'3d',id,1); 
a=diff_vert(om,1,2); a=a./dp; v.dom_dp=csampone(a,v,'3d',id,1);

v.qc=csampone(qc,v,'3d',id,1);
v.ql=csampone(ql,v,'3d',id,0);
v.qi=csampone(qi,v,'3d',id,0);
id0=(wa>0&qc>0);a=wa;a(~id0)=NaN; v.wa.cup0=getavar_smart(a,v,'3d',[0 1],pctile);
id0=(wa<0&qc>0);a=wa;a(~id0)=NaN; v.wa.cdn0=getavar_smart(a,v,'3d',[0 1],pctile);
id0=(qc>0);     a=wa;a(~id0)=NaN; v.wa.cld0=getavar_smart(a,v,'3d',[0 1],pctile);
id0=(qc==0);    a=wa;a(~id0)=NaN; v.wa.clr0=getavar_smart(a,v,'3d',[0 1],pctile);clear id0; 
...
%below is my own diagnostic of cloud fraction based on overlapping
do_cloud_cover=0
if (do_cloud_cover)
  p_mid=680; p_high=400; 
  a=ps; a(:,:,:)=1; qal1=a; qam1=a; qah1=a; qat1=a;
  for k = 2:v.nz
    a_km1=squeeze(qa(:,k-1,:,:));
    a_k  =squeeze(qa(:,k,  :,:));
    %maximum and random overlapping assumption
    %maximum (random) for 2 consecutive (separate) layers
    a=(1.-max(a_k(:,:,:),a_km1(:,:,:)))./(1.-a_km1(:,:,:));
    %a=1-a_k; %if using random overlapping assumption
    qat1(:,:,:)=qat1(:,:,:).*a;
    if (v.pf(k)<p_high)
      qah1(:,:,:)=qah1(:,:,:).*a;
    elseif (v.pf(k)<p_mid)
  ...
  qat1=1-qat1; v.atm.qat1 =getavar(qat1, v,'2d');
  qah1=1-qah1; v.atm.qah1 =getavar(qah1, v,'2d');
  qam1=1-qam1; v.atm.qam1 =getavar(qam1, v,'2d');
  qal1=1-qal1; v.atm.qal1 =getavar(qal1, v,'2d');
  a=qam1+qal1; v.atm.qlm1 =getavar(a,    v,'2d');
end
clear qa
%above is my own diagnostic of cloud fraction based on overlapping
...
ti=1; xi=nn; yi=nn; pctile=v.pctile;
wbin=[-100:5:100]; qcbin=[qc_th:0.0001:0.002]; 
aa=0.65*G*864; wa500=wa500*aa %change m/s to hPa/day and change sign
v.wa500_r96=onelev_txyavg(v,wa500,qc500,ti,xi,yi,wbin,wbin,wbin,pctile,qc_th)
%v.om500_r96=onelev_txyavg(v,om500,qc500,ti,xi,yi,wbin,wbin,wbin,pctile,qc_th)
...
%some online flux calculation=======
disp('do_flux...');
varn='wqt';       a=f{varn}(ts:te,zs:ze,:,:); v.atm.wqt_dyn  =csampone(a,v,'3d',id,0);%wqt=a;
varn='qtflx_dyn'; a=f{varn}(ts:te,zs:ze,:,:); v.atm.qtflx_dyn=csampone(a,v,'3d',id,0);%qtflx_dyn=a;
varn='qtflx_lin'; a=f{varn}(ts:te,:,:);       v.atm.qtflx_lin=csampone(a,v,'3d',id,0);%qtflx_lin=a;
varn='rainflx';   a=f{varn}(ts:te,:,:);       v.atm.rainflx  =csampone(a,v,'3d',id,0);%rainflx=a;
varn='solpflx';   a=f{varn}(ts:te,:,:);       v.atm.solpflx  =csampone(a,v,'3d',id,0);%solpflx=a;
varn='vtr';       a=f{varn}(ts:te,:,:);       v.atm.qrflx    =csampone(a,v,'3d',id,0);%qrflx=a;
...
fn=strcat(tpath,expn,'_',res,'_mon_',num2str(ts),'_',num2str(te),'.mat');fn
save(fn,'v'); return;
%save(fn,'v','-v7.3');
