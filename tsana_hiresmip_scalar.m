function v=tsana_hiresmip_scalar(o,tpath,expn,yr1,yr2,latlon,region,syr,nyr,myr,mod,opt)
...
mext='tsana.mat'; mpath='/work/miz/mat_hiresmip/';
...
v.sfc.sst_nino3=sst_nino3; v.missing_value=-1.e+10;v
...
if (exist(fname,'file') == 2)
  f=netcdf(fname,'nowrite'); 
  tmp=f{varn}(v.ts:v.te,v.ys:v.ye,v.xs:v.xe); close(f); swdnsfc=tmp;
...
if (exist(fname,'file') == 2)
  f=netcdf(fname,'nowrite'); 
  tmp=f{varn}(v.ts:v.te,v.ys:v.ye,v.xs:v.xe); close(f); swupsfc=tmp;
...
if (opt>2)
  ...
  fn=strcat(tpath,expn,'/',expn,'_',region,'_tsana.mat'); %save(fn,'v');
