function o=read_ceres_new(tpath,fn,latlon,t1,t2,s);
%latlon=[0 360 -90 90]; region='global'; wsize=1; 
%t1=11; t2=226; %for 2001Jan-2014Dec period
%tpath='/archive/Ming.Zhao/obs_for_am4p0_paper/CERES_EBAF_Ed4.1/';
%fn=strcat(tpath,'CERES_EBAF_Ed4.1_Subset_200003-202203.nc')
...
v.time=v.time(t1:t2); v.nt=length(v.time); v.ts=1; v.te=v.nt;
lonx=s.lon; latx =s.lat; aa=s.aa; o.fn_ceres=fn;

varn='toa_cre_sw_mon';
a=ncread(fn,varn,[1 1 1],[Inf Inf Inf]);
a=a(v.xs:v.xe,v.ys:v.ye,t1:t2); 
a=permute(a,[3 2 1]);
...
varn='toa_lw_clr_c_mon';
a=ncread(fn,varn,[1 1 1],[Inf Inf Inf]);
a=a(v.xs:v.xe,v.ys:v.ye,t1:t2); 
a=permute(a,[3 2 1]);
tmp=reshape(a,12,v.nt/12,v.nlat,v.nlon);
v.lwclrtoa_c_clm=squeeze(mean(tmp,2));
v.lwclrtoa_c_ann=squeeze(mean(v.lwclrtoa_c_clm,1));
v.lwclrtoa_c_sea=get4season(v.lwclrtoa_c_clm);
o.toa.lwnet_clr_c.all=interp_grid(tmp,lonx,latx,v.lon,v.lat,1);
[o.toa.lwnet_clr_c.al0 o.toa.lwnet_clr_c.ts.ann]=getts0(o.toa.lwnet_clr_c.all,s);

varn='solar_mon';
a=ncread(fn,varn,[1 1 1],[Inf Inf Inf]);
a=a(v.xs:v.xe,v.ys:v.ye,t1:t2); 
a=permute(a,[3 2 1]);
tmp=reshape(a,12,v.nt/12,v.nlat,v.nlon); swdn_toa=tmp;
v.swdntoa0_clm=squeeze(mean(tmp,2));
v.swdntoa0_ann=squeeze(mean(v.swdntoa0_clm,1));
v.swdntoa0_sea=get4season(v.swdntoa0_clm);

varn='toa_sw_clr_t_mon';
a=ncread(fn,varn,[1 1 1],[Inf Inf Inf]);
a=a(v.xs:v.xe,v.ys:v.ye,t1:t2); 
a=permute(a,[3 2 1]);
tmp=reshape(a,12,v.nt/12,v.nlat,v.nlon); tmp=swdn_toa-tmp;
...
varn='toa_sw_clr_c_mon';
a=ncread(fn,varn,[1 1 1],[Inf Inf Inf]);
a=a(v.xs:v.xe,v.ys:v.ye,t1:t2); 
a=permute(a,[3 2 1]);
tmp=reshape(a,12,v.nt/12,v.nlat,v.nlon); tmp=swdn_toa-tmp;
v.swclrtoa_c_clm=squeeze(mean(tmp,2));
v.swclrtoa_c_ann=squeeze(mean(v.swclrtoa_c_clm,1));
v.swclrtoa_c_sea=get4season(v.swclrtoa_c_clm);
o.toa.swnet_clr_c.all=interp_grid(tmp,lonx,latx,v.lon,v.lat,1);
[o.toa.swnet_clr_c.al0 o.toa.swnet_clr_c.ts.ann]=getts0(o.toa.swnet_clr_c.all,s);
...
varn='toa_net_clr_c_mon';
a=ncread(fn,varn,[1 1 1],[Inf Inf Inf]);
a=a(v.xs:v.xe,v.ys:v.ye,t1:t2); 
a=permute(a,[3 2 1]);
tmp=reshape(a,12,v.nt/12,v.nlat,v.nlon);
v.netradtoa_clr_c_clm=squeeze(mean(tmp,2));
v.netradtoa_clr_c_ann=squeeze(mean(v.netradtoa_clr_c_clm,1));
v.netradtoa_clr_c_sea=get4season(v.netradtoa_clr_c_clm);
o.toa.netrad_clr_c.all=interp_grid(tmp,lonx,latx,v.lon,v.lat,1);
%o.toa.netrad_clr_c.al0=getts(o.toa.netrad_clr_c.all,s)
[o.toa.netrad_clr_c.al0 o.toa.netrad_clr_c.ts.ann]=getts0(o.toa.netrad_clr_c.all,s);
...
o.toa.netrad_clr_c.clm=interp_grid(v.netradtoa_clr_c_clm,lonx,latx,v.lon,v.lat,1);
o.toa.netrad_clr_c.ann=interp_grid(v.netradtoa_clr_c_ann,lonx,latx,v.lon,v.lat,1);
o.toa.netrad_clr_c.sea=interp_grid(v.netradtoa_clr_c_sea,lonx,latx,v.lon,v.lat,1);
