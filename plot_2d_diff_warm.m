...
subplot(row,col,3); 
if (p.rel==1); 
  e=e./z0*100; p.vname=strcat(p.vname,'_rel');
  diff=(z1_mean-z0_mean)/z0_mean*100/dtas0;
  s2=sprintf('%s; \\DeltaG/G/\\DeltaTs=%5.2f%%/K',s2,diff);
else
  p.vname=strcat(p.vname,'_abs');  
  diff=(z1_mean-z0_mean)/z0_mean*100/dtas0;
  %s2=sprintf('%s; \\DeltaG/G/\\DeltaTs=%5.2f%%/K',s2,diff);
  s2=sprintf('%s; \\DeltaG/\\DeltaTs=%5.2f/K',s2,diff_pdk);
end;
