function [v]=readarmap_cre(tpath,expn,yr1,yr2,opt)
...
for m=1:length(d_end)
  ...
  v.p999=squeeze(prctile(a,99.9,1));
  nlat=length(a(1,:,1));
  nlon=length(a(1,1,:));
  for j=1:nlat
    for i=1:nlon
      id_p999(:,j,i)=(a(:,j,i)>=v.p999(j,i));
    end
  end
  v.id_p999_sum=squeeze(sum(id_p999,1));
  %end of new addition
  %IVTY
...
% $$$     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); s=get_amon_stat(a,id_day,1);
    v.lwcf_ar   (m,:,:)  =s.ar;    %ar total
    v.lwcf_aravg(m,:,:)  =s.aravg; %ar average
    v.lwcf_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
    ...
    v.swcf_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
    ...
    v.rlut_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
    ...
    v.rsut_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
    ...
    v.tas_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
    ...
    v.tasmax_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
    ...
    v.tasmin_arpct(m,:,:,:)=s.arpct; %[25 50 75 90 99 99.9] percentile;
...
fnmat=strcat(epath,expn,fext,'_readarmap_cre.mat')
