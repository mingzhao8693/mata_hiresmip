...
fnout='HadISST_sst_AWP_multidecadal.nc';

f =netcdf(fnin,'nowrite');
 hsst=f{'sst'}(1322:1573, 1:180, 1:360);
 nt=1573-1322+1;
 for ii=1:360;
 for jj=1:180; 
 for t=1:12;
 if rem(t, 12)~=0;
 hssts(t,jj,ii)=nanmean(hsst(rem(t,12):12:nt,jj,ii));
 else
 hssts(t,jj,ii)=nanmean(hsst(rem(t,12)+12:12:nt,jj,ii));
 end;
 end;
  end; %(jj)
end; %(ii)

   fid=fopen('mask.dat', 'rb');
   amask=fread(fid, [360,180], 'real*4');
   bmask=permute(amask, [2 1]);
   bmask=bmask*10e39/0.4601;
   lmask=logical(bmask);
   bmask(bmask==0)=NaN;
   %bmask=bmask*10e39/0.4601;
...
if 1==1;
 ...
 for jj=1:180;
 for t=8:10;
 hssts(t,jj,ii)=hssts(t,jj,ii)+coe1(jj,ii); %add anomaly
 end;
 end; %(jj)
 end; %(ii)
...
for t=1:12;
v.sst_elnino(t,:,:)=hssts(t,:,:); %pass to v.sst_elnino for output
end;
...
nc{'sst'}=ncfloat('time','lat','lon'); nc{'sst'}(:,:,:)=v.sst_elnino(:,:,:);
...
nc{'nrecords'} = ncint();  nc{'nrecords'}(:)=12; % error line
