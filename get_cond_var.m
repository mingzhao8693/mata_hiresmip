function s=get_cond_var(a,id,pct,opt,id99,id999,ip99,ip999)
...
if exist('opt')
  if (opt==0)
    ...
    idx=id99;  b=a.*idx; d=b; d(~idx)=NaN;
    s.idavg_p99 =squeeze(nanmean(d,1));
    s.idstd_p99 =squeeze(nanstd (d,1));
    idx=id999; b=a.*idx; d=b; d(~idx)=NaN;
    s.idavg_p999=squeeze(nanmean(d,1));
    s.idstd_p999=squeeze(nanstd (d,1));
    idx=ip99;  b=a.*idx; d=b; d(~idx)=NaN;
    s.nonid_p99 =squeeze(nanmean(d,1));
    idx=ip999; b=a.*idx; d=b; d(~idx)=NaN;
    s.nonid_p999=squeeze(nanmean(d,1));
  elseif (opt==3)
    b=a.*id; d=b; d(~id)=NaN;
    s.idwei=squeeze(mean   (b,1));
    s.idavg=squeeze(nanmean(d,1));
    ...
    for i=1:length(s.idpct_values)
      if (i==1) %compute lower quarter mean
	x=squeeze(s.idpct(i,:,:)); 
	...
	s.idpct_wei(i,:,:)=squeeze(mean(dd,1));
      else %computer upper quarter means
	x=squeeze(s.idpct(i,:,:)); 
	x=repmat(x,[1 1 nt]); x=shiftdim(x,2);
	idx=(d>x); dd=d; dd(~idx)=NaN;
	s.idpct_avg(i,:,:)=squeeze(nanmean(dd,1));
