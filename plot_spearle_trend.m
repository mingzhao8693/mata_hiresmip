y1=1979; y2=2020; alpha=0.8; %y1=2021; y2=2070; alpha=0.8;
v=get_trend_any(vm, y1, y2, alpha)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if p.do_norm
  ...
  p.unit='\rm{K dec^{-1}}'; p.vname=a; cmin=-0.3; cmax=0.3; %no normalization
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
row=4; col=3; cmap=bluewhitered_miz(256); co='k';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lon=o.lon; lat=o.lat; alpha=0.8; y1=1979; y2=2020;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%compute the hadisst SST trend
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ia=1870; ib=2021; Y=[ia:ib]';
a=vm(1).tr.sst.hadisst.ts.sst;
b=vm(1).tr.sst.hadisst.ts.ice;
...
obs.xt=Y(i1:i2);
a=a(i1:i2,:,:,:); %obs.sst=a; obs.sst_clm=squeeze(mean(a,1));
b=b(i1:i2,:,:,:); %obs.ice=b; obs.ice_clm=squeeze(mean(b,1));
obs.sst(:,1:12,:,:)=a;
obs.ice(:,1:12,:,:)=b;
...
for m=1:length(obs.sst(1,:,1,1))
  b=squeeze(obs.sst(:,m,:,:));
  x=get_trend_TSR_simple(v.s,b,obs.xt,alpha);
  obs.trend(m,:,:)=x.trend;
  obs.mean (m,:,:)=x.mean;
end
%m=m+1; b=squeeze(mean(a,2));
%x=get_trend_TSR_simple(v.s,b,obs.xt,alpha);
%obs.trend(m,:,:)=x.trend;
%obs.mean (m,:,:)=x.mean;
...
for i = 1:mod_num
  ...
  a=a(i1:i2,:,:,:); %mod(i).sst=a; mod(i).sst_clm=squeeze(mean(a,1));
  b=b(i1:i2,:,:,:); %mod(i).ice=b; mod(i).ice_clm=squeeze(mean(b,1));
  sst(i,:,1:12,:,:)=a;
  ice(i,:,1:12,:,:)=b;
...
%  a=mod(i).sst_clm;
%  b=mod(i).ice_clm;
%  mod(i).sst_clm(13,:,:)=squeeze(mean(a,1));
%  mod(i).ice_clm(13,:,:)=squeeze(mean(b,1));
%  mod(i).sst_clm(14,:,:)=squeeze((a(12,:,:)+a(1,:,:)+a(2,:,:))/3);
%  mod(i).ice_clm(14,:,:)=squeeze((b(12,:,:)+b(1,:,:)+b(2,:,:))/3);
%  mod(i).sst_clm(15,:,:)=squeeze(mean(a(6:8,:,:),1));
%  mod(i).ice_clm(15,:,:)=squeeze(mean(b(6:8,:,:),1));
end
i=mod_num+1;
...
%a=mod(i).sst_clm;
%b=mod(i).ice_clm;
%mod(i).sst_clm(13,:,:)=squeeze(mean(a,1)); %annual mean
%mod(i).ice_clm(13,:,:)=squeeze(mean(b,1)); %annual mean
%mod(i).sst_clm(14,:,:)=squeeze((a(12,:,:)+a(1,:,:)+a(2,:,:))/3);%DJF
%mod(i).ice_clm(14,:,:)=squeeze((b(12,:,:)+b(1,:,:)+b(2,:,:))/3);%DJF
%mod(i).sst_clm(15,:,:)=squeeze(mean(a(6:8,:,:),1)); %JJA
%mod(i).ice_clm(15,:,:)=squeeze(mean(b(6:8,:,:),1)); %JJA
for i = 1:mod_num+1
  sst=mod(i).sst; disp('mode_num:');i
  for m=1:length(sst(1,:,1,1)); disp('m=');m
    a=squeeze(mod(i).sst(:,m,:,:));
    x=get_trend_TSR_simple(s,a,xt,alpha);
    mod(i).trend(m,:,:)=x.trend;
    mod(i).mean (m,:,:)=x.mean;
  end
  mod(i).xt=xt; 
%  m=m+1;
%  a=squeeze(mean(mod(i).sst,2));
%  x=get_trend_TSR_simple(s,a,xt,alpha);
%  mod(i).trend(m,:,:)=x.trend;
%  mod(i).mean (m,:,:)=x.mean;
%  mod(i).xt=xt;
...
clear v;
v.mod=mod; v.obs=obs; v.id=id; v.idm=idm; v.s=s; v.p=p;
fn='/work/miz/mat_spear/spear_sst_trend_1979_2020_more.mat'; save(fn,'v','-v7.3');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%load and plot%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tpath='/work/miz/mat_spear/'; y1=1979; y2=2020;
fn=strcat('spear_sst_trend_',num2str(y1),'_',num2str(y2),'.mat');
fn=strcat(tpath,fn); load(fn);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
s=v.s; aa=s.aa; lm=s.lm; lon=s.lon; lat=s.lat;
i=31; %choose individual member or ensemble (i=31)
mod=v.mod(i).trend; mod_ice=v.mod(i).ice_clm;
obs=v.obs.trend;    obs_ice=v.obs.ice_clm; ice_th=0.5;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p.vname='sst_trend_1979_2020_mon_norm'; p.mod_name='spear';
p.x1=0; p. x2=360; p.y1=-90; p.y2=90;
p.cmin=-0.3; p.cmax=0.3; p.xsize=1000; p.ysize=1000;
p.do_norm=1; p.unit='K/K'; %p.unit='K/dec';
pms=[ 0, 0, p.xsize, p.ysize]*1.2; fsize=6; 
handle=figure('Position',pms,'visible','on'); 
row=5; col=3; cmap=bluewhitered_miz(256); 
...
for i=1:15
  ...
  b0=mean(b(id).*aa0); idb=id;
  if p.do_norm   
    a=(a-a0)/a0; b=(b-b0)/b0; cmin=-1.5; cmax=1.5;
  end
  subplot(row, col, i); a(~ida)=NaN; colormap(cmap);
  pcolor(lon,lat,a); hold on; shading flat; caxis([cmin, cmax]);
  contour(lon,lat,lm,1,co); 
  set(gca,'FontSize',fsize); axis([p.x1 p.x2 p.y1 p.y2]);
  id=ida & idb; c=corrcoef(a(id),b(id));
  titl=sprintf('%s (avg=%4.3fK; r=%4.3f)',mn{i},a0,c(1,2));
  title(titl,'FontSize',fsize);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p.cmin=-0.3; p.cmax=0.3; p.xsize=1100; p.ysize=1000;
p.x1=0; p. x2=360; p.y1=-90; p.y2=90;
pms=[ 0, 0, p.xsize, p.ysize]*1.2; fsize=6; 
handle=figure('Position',pms,'visible','on'); 
...
co='k'; cmin=p.cmin; cmax=p.cmax;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mod_num=30; idm=v.idm; id=v.id;
s=v.s; lon=s.lon; lat=s.lat; lm=s.lm; aa=s.aa; aa0=aa(id)/mean(aa(id));
m=13; ob=squeeze(v.obs.trend(m,:,:));
for i = 1:mod_num+1
  a=v.mod(i).trend; a=squeeze(a(m,:,:));
  subplot(row, col, i); colormap(cmap); a(idm)=NaN;
  pcolor(lon,lat,a); hold on; shading flat; caxis([cmin, cmax]);
  contour(lon,lat,lm,1,co);
  set(gca,'FontSize',fsize); axis([p.x1 p.x2 p.y1 p.y2]);
  a0=mean(mean(a(id).*aa0));
  c=corrcoef(a(id),ob(id));
  titl=sprintf('M%02d (avg=%4.2fK; r=%4.2f)',i,a0,c(1,2));
  title(titl,'FontSize',fsize);
end
titl=sprintf('ENS mean (avg=%4.2fK; r=%4.2f)',a0,c(1,2));
