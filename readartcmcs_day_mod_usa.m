function [v]=readartcmcs_day_mod_usa(tpath,expn,yr1,yr2,pct,opt,diag,latlon)
...
%expn ='ERAI_6h_DATA'; yr1=2015; yr2=2015; opt='TC';
%pct  =[99 99.9]; latlon=[180 340 10 90]; diag=0;
%tpath='/archive/Ming.Zhao/awg/2022.03/'; diag=0; latlon=[180 340 10 90];pct=[99 99.9];
%expn='c192L33_am4p0_amip_HIRESMIP_nudge_wind_1951_2020'; yr1=1985;yr2=1985;opt='AR';
...
v=readts_grid_2d(tpath,expn,fn, latlon); 
%v.lon=ncread(fn,'lon'); lat=ncread(fn,'lat'); v.lat=lat;
a=ncread(fn,'land_mask'); v.lm_org=a';
a=a(v.xs:v.xe,v.ys:v.ye); a=a'; 
%figure; pcolor(a); shading flat; colorbar;
...
mon=[31 28 31 30 31 30 31 31 30 31 30 31]; ddd=cumsum(mon); %yea=[365]; ddd=cumsum(yea);
...
for m=1:length(d_end)
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400.;v.daily(m).prday=a; size(a) %unit:mm/day
  ...
  v.frqday(m,:,:)=squeeze(mean(id,1)); v.daily(m).id=id;
  ...
  for j=1:nlat
    for i=1:nlon
      ...
      v.prday_fp99_from_id(m,j,i) =sum(idboth)/sum(idallp);
%      id99 (:,j,i)=idboth;              %P99 and ID
%      ip99 (:,j,i)=idallp & ~id(:,j,i); %P99 and NonID    
      idallp=(a(:,j,i)>=P999(j,i));
      idboth=idallp & id(:,j,i);
      v.prday_fp999_from_id(m,j,i)=sum(idboth)/sum(idallp);
%      id999(:,j,i)=idboth;
%      ip999(:,j,i)=idallp & ~id(:,j,i);      
    end
  end 
  v.id_sum(m,:,:) =squeeze(sum(id,1));
...
%  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400; 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa);
  ...
  idxx=s.idx; clear s;
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
%  v.prday_nonid_ts_day_lat(m,:,:)  =s.nonid_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='prw'; ff='day'; exd=strcat('/',atmos_data_dir,'/daily/'); exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff);
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
%  v.prw_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='ivty'; ff='day'; exd=strcat('/',atmos_data_dir,'/daily_avg/'); exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  ivty=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff); 
  s=get_cond_var_new_usa(ivty,mon(m),id,pct,v.aa,0,idxx);
...
%  v.ivty_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='ivtx'; ff='day'; exd=strcat('/',atmos_data_dir,'/daily_avg/'); exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  ivtx=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff);
  s=get_cond_var_new_usa(ivtx,mon(m),id,pct,v.aa,0,idxx);
...
%  v.ivt_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='uas'; ff='day'; exd=strcat('/',atmos_data_dir,'/daily/'); exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  u_ref=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff);
  s=get_cond_var_new_usa(u_ref,mon(m),id,pct,v.aa,0,idxx);
  v.u_ref_av        (m,:,:)  =s.av;         %all average
  v.u_ref_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;
  v.u_ref_idwei     (m,:,:)  =s.idwei;      %id total
  v.u_ref_idmax     (m,:,:)  =s.idmax;      %id max
  v.u_ref_idavg     (m,:,:)  =s.idavg;      %id average
  v.u_ref_idstd     (m,:,:)  =s.idstd;      %id average
  v.u_ref_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.u_ref_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.u_ref_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.u_ref_idx_std   (m,:,:,:)=s.idx_std;    %id average
%  v.u_ref_av_ts_day       (m,:)   =s.av_ts_day;
%  v.u_ref_idwei_ts_day    (m,:)   =s.idwei_ts_day;
%  v.u_ref_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
%  v.u_ref_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  varn='vas'; ff='day'; exd=strcat('/',atmos_data_dir,'/daily/'); exf1='atmos_cmip.'; exf2='0101-'; exf3='1231.';
  v_ref=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff);
  s=get_cond_var_new_usa(v_ref,mon(m),id,pct,v.aa,0,idxx);
  v.v_ref_av        (m,:,:)  =s.av;         %all average
  v.v_ref_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;
  v.v_ref_idwei     (m,:,:)  =s.idwei;      %id total
  v.v_ref_idmax     (m,:,:)  =s.idmax;      %id max
  v.v_ref_idavg     (m,:,:)  =s.idavg;      %id average
  v.v_ref_idstd     (m,:,:)  =s.idstd;      %id average
  v.v_ref_nonid_avg (m,:,:)  =s.nonid_avg;      %id average
  v.v_ref_nonid_std (m,:,:)  =s.nonid_std;      %id average
  v.v_ref_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.v_ref_idx_std   (m,:,:,:)=s.idx_std;    %id average
%  v.v_ref_av_ts_day       (m,:)   =s.av_ts_day;
%  v.v_ref_idwei_ts_day    (m,:)   =s.idwei_ts_day;
%  v.v_ref_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
%  v.v_ref_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %wind_ref%%%%%%%%%%%%%%%%%%%
  a=sqrt(u_ref.*u_ref+v_ref.*v_ref); clear u_ref v_ref;
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
  v.wind_ref_av        (m,:,:)   =s.av;         %all average
  v.wind_ref_pct       (m,:,:,:) =s.pct;        %[25 50 75 90 99 99.9] percentile;
  v.wind_ref_idwei     (m,:,:)   =s.idwei;      %id total
  v.wind_ref_idmax     (m,:,:)   =s.idmax;      %id max
  v.wind_ref_idavg     (m,:,:)   =s.idavg;      %id average
  v.wind_ref_idstd     (m,:,:)   =s.idstd;      %id average
  v.wind_ref_nonid_avg (m,:,:)   =s.nonid_avg;  %id average
  v.wind_ref_nonid_std (m,:,:)   =s.nonid_std;  %id average
  v.wind_ref_idx_avg   (m,:,:,:)=s.idx_avg;    %id average
  v.wind_ref_idx_std   (m,:,:,:)=s.idx_std;    %id average
%  v.wind_ref_av_ts_day       (m,:)   =s.av_ts_day;
%  v.wind_ref_idwei_ts_day    (m,:)   =s.idwei_ts_day;
%  v.wind_ref_av_ts_day_lat   (m,:,:) =s.av_ts_day_lat;
%  v.wind_ref_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
%epath=strcat(tpath,expn,'/');
epath=strcat('/work/miz/mat_hiresmip/usa/',expn,'/');
%fext =strcat('_',num2str(yr1),'_',num2str(yr2),'_',num2str(v.d_beg(m)),'_',num2str(v.d_end(m)));
fext =strcat('_',num2str(yr1),'_',num2str(yr2));
fnmat=strcat(epath,expn,fext,'_',opt,'_diag',num2str(diag),'_readartcmcs_day_usa.mat')
