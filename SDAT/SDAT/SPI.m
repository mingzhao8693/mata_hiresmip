...
if length(td(td>=0))/length(td)~=1
   ...
   A1=[];
   for i=1:sc,  
   A1=[A1,td(i:length(td)-sc+i)]; %A1 containing copy of column td shifting by 1 to sc
   end
   Y=sum(A1,2); %Y containing accumulation of td equivalent to (sc point) running mean average of precip
   ...
   for k=1:12; k=1; %why 12? is it because 12 month form annual cycle
     d=Y(k:12:nn);
...
xlabel('time step'); %Modify "time step"
