function [v]=readartcmcs_day_cre(tpath,expn,yr1,yr2,pct,opt,diag)
...
%  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff)*86400; 
  ...
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); rlut=a;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  ...
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); lwcf=a-rlut; clear rlut;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  v.rlutcs_av        (m,:,:)  =s.av;         %all average
  v.rlutcs_pct       (m,:,:,:)=s.pct;        %[25 50 75 90 99 99.9] percentile;
  v.rlutcs_idwei     (m,:,:)  =s.idwei;      %id total
...
% $$$   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); rsut=a;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
  ...
  v.rsutcs_idwei     (m,:,:)  =s.idwei;      %id total
  ...
  a=readallyear_mon(v,exd,varn,exf1,exf2,exf3,m,ff); a=a-rsut;
  s=get_cond_var_new(a,id,pct,v.aa,0,idxx);
