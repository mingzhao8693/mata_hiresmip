...
pms=[ 0, 0, 1400, 1000]*1.2; fsize=9; row=4; col=4;
...
for r = 1:4 % Rows: Present, SPEAR, Observed, Diff
    for c = 1:4 % Cols: TAS, VPD, TWB, RH
        ...
        if c == 1 % TAS Column
            ...
            elseif r==2; a=p.v1; b=p.dv1; isig=p.isig1; sx=let(idx)+p.s1; cax=[p.cmin1 p.cmax1];
            elseif r==3; a=p.v2; b=p.dv2; isig=p.isig2; sx=let(idx)+p.s2; cax=[p.cmin1 p.cmax1];
            else; a=p.v2-p.v1; b=p.dv2-p.dv1; sx=let(idx)+'(c) minus (b)'; cax=[p.cmin1 p.cmax1];
            end;
            unit_val = p.unit1; unit_str = p.unit1_bar; % Shared for TAS
            if r==1; unit_val = p.unit0; unit_str = p.unit0_bar; end
            
        elseif c == 2 % VPD Column
            if r==1; a=p.v3; b=p.dv3; sx=let(idx)+p.s3; cax=[p.cmin3 p.cmax3];
            elseif r==2; a=p.v4; b=p.dv4; isig=p.isig4; sx=let(idx)+p.s4; cax=[p.cmin4 p.cmax4];
            elseif r==3; a=p.v5; b=p.dv5; isig=p.isig5; sx=let(idx)+p.s5; cax=[p.cmin4 p.cmax4];
            else; a=p.v5-p.v4; b=p.dv5-p.dv4; sx=let(idx)+'(g) minus (f)'; cax=[p.cmin4 p.cmax4];
            end;
            unit_val = p.unit4; unit_str = p.unit4_bar;
            if r==1; unit_val = p.unit3; unit_str = p.unit3_bar; end

        elseif c == 3 % TWB Column
            if r==1; a=p.v6; b=p.dv6; sx=let(idx)+p.s6; cax=[p.cmin6 p.cmax6];
            elseif r==2; a=p.v7; b=p.dv7; isig=p.isig7; sx=let(idx)+p.s7; cax=[p.cmin7 p.cmax7];
            elseif r==3; a=p.v8; b=p.dv8; isig=p.isig8; sx=let(idx)+p.s8; cax=[p.cmin7 p.cmax7];
            else; a=p.v8-p.v7; b=p.dv8-p.dv7; sx=let(idx)+'(k) minus (j)'; cax=[p.cmin7 p.cmax7];
            end;
            unit_val = p.unit7; unit_str = p.unit7_bar;
            if r==1; unit_val = p.unit6; unit_str = p.unit6_bar; end

        elseif c == 4 % RH Column
            if r==1; a=p.v9; b=p.dv9; sx=let(idx)+p.s9; cax=[p.cmin9 p.cmax9];
            elseif r==2; a=p.v10; b=p.dv10; isig=p.isig10; sx=let(idx)+p.s10; cax=[p.cmin10 p.cmax10];
            elseif r==3; a=p.v11; b=p.dv11; isig=p.isig11; sx=let(idx)+p.s11; cax=[p.cmin10 p.cmax10];
            else; a=p.v11-p.v10; b=p.dv11-p.dv10; sx=let(idx)+'(o) minus (n)'; cax=[p.cmin10 p.cmax10];
            end;
            unit_val = p.unit10; unit_str = p.unit10_bar;
            if r==1; unit_val = p.unit9; unit_str = p.unit9_bar; end
        end
...
expn='pattern_effect_16panel_rotated'; visfig='off'; figpath='./fig_land/';
