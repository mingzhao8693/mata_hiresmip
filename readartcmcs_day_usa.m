function [v]=readartcmcs_day_usa(tpath,expn,yr1,yr2,pct,opt,diag,latlon)
...
%expn ='c192L33_am4p0_amip_HIRESMIP_H8_newpp'; yr1=1980; yr2=1981; opt='OP';
%pct  =[99 99.9]; diag=1; latlon=[180 340 10 90]; diag=0;

fn=strcat(tpath,expn,'/atmos.static.nc'); 
v=readts_grid_2d(tpath,expn,fn, latlon); 
%v.lon=ncread(fn,'lon'); v.lat=ncread(fn,'lat');
...
for m=1:length(d_end)
  ...
  [a time]=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff);
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400; %unit:mm/day
...
% $$$   v.tas_nonid_ts_day_lat(m,:,:)  =s.nonid_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev)*864;
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
% $$$   v.w500_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
% $$$   v.t850_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff); ttcf=a;
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
% $$$   v.lwcf_nonid_ts_day_lat(m,:,:)  =s.nonid_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff); ttcf=ttcf+a;
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
if (diag==2)
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400.; %mm/day
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); prc=a; clear a;
...
% $$$   v.prc_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)/LV0*86400;%mm/day
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); cvh=a; clear a;
...
% $$$   v.cvh_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); clear a;%save dcv!
...
% $$$   s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); clear a;
...
% $$$   v.pecv_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400.; 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); lsh=a; clear a;
...
% $$$   v.qlcond_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400.; 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); lsh=lsh+a; clear a;
...
% $$$   v.lsh_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff)*86400.; 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx); prl=a; clear a;
...
%  if (diag==2); break; end
  ...
  s=get_cond_var_new_usa(ivty,id,pct,v.aa,0,idxx);
...
% $$$   v.ivty_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  s=get_cond_var_new_usa(ivtx,id,pct,v.aa,0,idxx);
...
% $$$   v.prsn_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff); a=a/LV0*86400.;
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);
...
% $$$   v.w850_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);u850=a;
...
% $$$   v.u850_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);v850=a;
...
% $$$   v.w500_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);u500=a;
...
% $$$   v.u500_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);v500=a;
...
% $$$   v.w250_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);u250=a;
...
% $$$   v.u250_idwei_ts_day_lat(m,:,:) =s.idwei_ts_day_lat;
  ...
  a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff,lev); 
  s=get_cond_var_new_usa(a,mon(m),id,pct,v.aa,0,idxx);v250=a;
...
% $$$     a=readallyear_mon_usa(v,exd,varn,exf1,exf2,exf3,m,ff); s=get_cond_var(a,mon(m),id,pct,0,id99,id999,ip99,ip999);
% $$$     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
